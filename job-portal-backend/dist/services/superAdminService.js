...
class SuperAdminService {
    ...
    async createAdmin(adminData, createdBy) {
        ...
        if (existingUser) {
            throw new errorMiddleware_1.ValidationError('User with this email already exists');
        }
        const userType = await this.prisma.userType.findFirst({
            where: { type_name: role }
        });
        if (!userType) {
            throw new errorMiddleware_1.ValidationError('Invalid admin role');
        }
    ...
    async getAdminById(adminId) {
        ...
        if (!admin) {
            throw new errorMiddleware_1.NotFoundError('Admin user');
        }
        return admin;
    }
    async updateAdmin(adminId, updateData, updatedBy) {
        ...
        if (adminId === updatedBy && updateData.role === 'Admin') {
            throw new errorMiddleware_1.ValidationError('You cannot demote your own Super Admin account');
        }
        if (updateData.role && updateData.role !== admin.user_type.type_name) {
            ...
            if (!newUserType) {
                throw new errorMiddleware_1.ValidationError('Invalid role');
            }
    ...
    async deleteAdmin(adminId, deletedBy) {
        if (adminId === deletedBy) {
            throw new errorMiddleware_1.ValidationError('You cannot delete your own account');
        }
        const admin = await this.prisma.user.findFirst({
            where: {
                id: adminId,
        ...
        if (isSuperAdmin && superAdminCount <= 1) {
            throw new errorMiddleware_1.ValidationError('Cannot delete the last Super Admin account');
        }
    ...
    async toggleMaintenanceMode(enabled, updatedBy) {
        const settings = await this.getSystemSettings();
        const updated = await this.updateSystemSettings({ maintenance_mode: enabled }, updatedBy);
        return {
    ...
    // ========== DATA EXPORT ==========
    async exportAllUsers(format = 'json') {
    ...
    async exportAllJobs(format = 'json') {
    ...
    async createUserType(typeName, createdBy) {
        const existing = await this.prisma.userType.findUnique({
            where: { type_name: typeName }
        });
        if (existing) {
            throw new errorMiddleware_1.ValidationError('User type already exists');
        }
