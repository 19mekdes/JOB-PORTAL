...
app.put("/api/auth/profile", authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const { title, linkedin_url, github_url, portfolio_url, availability, full_name, phone, location, bio, skills, experience, education, } = req.body;
        ...
        if (experience !== undefined) {
            updateData.experience = (0, safeJson_1.safeJsonParse)(experience);
        }
        // Education
        if (education !== undefined) {
            updateData.education = (0, safeJson_1.safeJsonParse)(education);
        }
...
app.put("/api/profile/update-info", authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const { title, linkedin_url, github_url, portfolio_url, availability, full_name, phone, location, bio, } = req.body;
...
app.get("/api/super-admin/companies", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
...
// GET single company
app.get("/api/super-admin/companies/:id", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// Verify company
app.put("/api/super-admin/companies/:id/verify", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.put("/api/super-admin/companies/:id/status", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// Delete company
app.delete("/api/super-admin/companies/:id", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// Get all backups
app.get("/api/admin/backups", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        let backups = [];
        try {
...
app.get("/api/admin/backups/stats", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        let backups = [];
        try {
            backups = await prisma.backup.findMany();
...
// Create backup
app.post("/api/admin/backups/create", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
...
// Download backup
app.get("/api/admin/backups/:id/download", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// Delete backup
app.delete("/api/admin/backups/:id", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        try {
...
// Restore backup
app.post("/api/admin/backups/:id/restore", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get("/api/admin/audit-logs", authMiddleware, isAdmin, async (req, res) => {
    try {
...
app.get("/api/admin/audit-logs/export", authMiddleware, isAdmin, async (req, res) => {
    try {
        const currentUser = await prisma.user.findUnique({
            where: { id: req.user.id },
            include: { user_type: true },
...
// 1. UPDATE ADMIN 
app.put("/api/admin/users/:userId", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
...
app.post("/api/admin/users/:userId/reset-password", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { userId } = req.params;
...
// 3. DELETE ADMIN 
app.delete("/api/admin/users/:userId", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { userId } = req.params;
...
// ========== ADMIN APPROVE JOB ==========
app.put("/api/admin/jobs/:id/approve", authMiddleware, isAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
// ========== ADMIN REJECT JOB ==========
app.put("/api/admin/jobs/:id/reject", authMiddleware, isAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
app.delete("/api/jobs/:id", authMiddleware, async (req, res) => {
    try {
        ...
        const isAdmin = user?.user_type?.type_name === "Admin" ||
...
app.get("/api/employer/dashboard", authMiddleware, async (req, res) => {
    try {
        ...
        const recentApplications = allApplications.slice(0, 5).map((app) => ({
            ...
            applied_days_ago: Math.floor((Date.now() - new Date(app.applied_at).getTime()) /
                (1000 * 60 * 60 * 24)),
        }));
        res.json({
            success: true,
...
app.put("/api/admin/jobs/:id/moderate", authMiddleware, isAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get("/api/employer/profile", authMiddleware, isEmployer, async (req, res) => {
    try {
...
app.put("/api/employer/profile", authMiddleware, isEmployer, async (req, res) => {
    try {
        const { company_name, company_description, website, location, phone, company_size, industry, } = req.body;
...
app.post("/api/employer/logo", authMiddleware, isEmployer, uploadImage.single("logo"), async (req, res) => {
    try {
...
app.delete("/api/employer/logo", authMiddleware, isEmployer, async (req, res) => {
    try {
...
app.put("/api/notifications/preferences", authMiddleware, async (req, res) => {
    try {
        const { email_notifications, application_alerts, job_alerts, marketing_emails, push_notifications, status_changes, weekly_digest, } = req.body;
...
app.get("/api/admin/test", authMiddleware, async (req, res) => {
...
app.get("/api/admin/settings", authMiddleware, isAdmin, async (req, res) => {
    try {
...
app.put("/api/admin/settings", authMiddleware, isAdmin, async (req, res) => {
    try {
...
app.post("/api/admin/settings/test-email", authMiddleware, isAdmin, async (req, res) => {
    try {
...
app.post("/api/super-admin/admins/:adminId/reset-password", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { adminId } = req.params;
        const { newPassword } = req.body;
        if (!newPassword || newPassword.length < 6) {
...
app.put("/api/super-admin/admins/:adminId/suspend", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { adminId } = req.params;
...
app.put("/api/super-admin/admins/:adminId/activate", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        const { adminId } = req.params;
        const updatedAdmin = await prisma.user.update({
            where: { id: adminId },
...
// Bulk action on jobs
app.post("/api/admin/jobs/bulk-action", authMiddleware, async (req, res) => {
    try {
...
app.post("/api/admin/users/:userId/reset-password", authMiddleware, async (req, res) => {
    try {
      ...
      `;
        ...
        res.json({
            success: true,
            message: `Password reset successfully for ${targetUser.email}${emailSent
...
app.put("/api/notifications/preferences", authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const { email_notifications, application_alerts, marketing_emails, push_notifications, status_changes, weekly_digest, } = req.body;
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="padding: 30px; background: #f9fafb;">
              ...
              <div style="background: #e0e7ff; padding: 15px; border-radius: 8px; margin: 20px 0;">
                ...
                ${coverLetter
                    ? `<p><strong>Cover Letter:</strong> ${coverLetter.substring(0, 200)}${coverLetter.length > 200 ? "..." : ""}</p>`
                    : ""}
              </div>
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div style="background: #e0e7ff; padding: 15px; border-radius: 8px; margin: 20px 0;">
                ...
                ${coverLetter
                ? `<p style="margin: 5px 0;">📝 Cover Letter: ${coverLetter.substring(0, 200)}${coverLetter.length > 200 ? "..." : ""}</p>`
                : ""}
              </div>
              <div style="text-align: center;">
...
app.put("/api/applications/:id/status", authMiddleware, async (req, res) => {
    try {
      ...
      `;
        ...
        // Create notification
        await createNotification(application.seeker.user_id, "Application Status Updated", `Your application for "${application.job.title}" has been ${newStatus.status_name.toLowerCase()}`, "application_update", { application_id: id, status: newStatus.status_name });
        res.json({
            success: true,
...
// ========== TEST EMAIL ENDPOINT ==========
app.post("/api/notifications/test-email", authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        // Get user details
        const user = await prisma.user.findUnique({
...
app.get("/api/admin/analytics-debug", authMiddleware, async (req, res) => {
    try {
        const totalUsers = await prisma.user.count();
        const totalJobs = await prisma.jobPost.count();
        const totalApplications = await prisma.jobApplication.count();
        ...
        res.json({
            ...
            data: {
                ...
                totalViews: (await prisma.jobPost.aggregate({ _sum: { views_count: true } }))
                    ._sum.views_count || 0,
                averageViewsPerJob: totalJobs > 0
                    ? Math.round((await prisma.jobPost.aggregate({
                        _sum: { views_count: true },
                ...
                conversionRate: totalApplications > 0
                    ? Math.round(((await prisma.jobApplication.count({
                        where: { status: { status_name: "Accepted" } },
                    })) /
                        totalApplications) *
                        100)
...
app.get("/api/super-admin/analytics", authMiddleware, async (req, res) => {
    try {
      ...
      LIMIT 10
    `;
        const jobsByIndustry = jobsByIndustryRaw.map((item) => ({
            name: item.name,
      ...
      LIMIT 10
    `;
        const topEmployers = topEmployersRaw.map((item) => ({
            name: item.name,
...
app.get("/api/super-admin/settings", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
...
app.put("/api/super-admin/settings", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
        console.log("⚙️ Super Admin updating system settings");
        const { 
        // General Settings
        siteName, siteDescription, siteLogo, siteFavicon, contactEmail, supportEmail, timezone, dateFormat, maintenanceMode, maintenanceMessage, 
        // User Settings
        enableRegistration, emailVerification, defaultUserRole, maxUsers, autoApproveEmployers, allowSocialLogin, 
        // Job Settings
        requireJobApproval, maxJobsPerEmployer, jobExpiryDays, allowJobEditing, featuredJobPrice, allowFreeJobs, maxJobTags, 
        // Application Settings
        maxApplicationsPerSeeker, requireCoverLetter, requireResumeUpload, applicationCooldown, allowWithdrawApplication, notifyEmployerOnApplication, 
        // Security Settings
        passwordMinLength, requireUppercase, requireNumbers, requireSpecialChars, sessionTimeout, maxLoginAttempts, twoFactorAuth, lockoutDuration, 
        // Notification Settings
        emailNotifications, pushNotifications, notifyOnNewJob, notifyOnApplication, notifyOnStatusChange, weeklyDigest, adminAlertEmails, 
        // Email Settings
        smtpHost, smtpPort, smtpUser, smtpPassword, smtpEncryption, fromEmail, fromName, 
        // API Settings
        googleClientId, googleClientSecret, linkedinClientId, linkedinClientSecret, recaptchaSiteKey, recaptchaSecretKey, 
        // Backup Settings
        autoBackup, backupFrequency, backupRetention, logRetentionDays, 
        // File Settings
        allowedFileTypes, maxFileSizeMb, } = req.body;
        ...
        if (siteFavicon !== undefined)
            updateData.site_favicon = siteFavicon;
        if (contactEmail !== undefined)
            updateData.contact_email = contactEmail;
        if (supportEmail !== undefined)
            updateData.support_email = supportEmail;
        ...
        if (maxJobTags !== undefined)
            updateData.max_job_tags = maxJobTags;
        // Application Settings
        ...
        if (weeklyDigest !== undefined)
            updateData.weekly_digest = weeklyDigest;
        if (adminAlertEmails !== undefined)
            updateData.admin_alert_emails = adminAlertEmails;
        // Email Settings
        if (smtpHost !== undefined)
            updateData.smtp_host = smtpHost;
        if (smtpPort !== undefined)
            updateData.smtp_port = smtpPort;
        if (smtpUser !== undefined)
            updateData.smtp_user = smtpUser;
        ...
        if (fromEmail !== undefined)
            updateData.from_email = fromEmail;
        ...
        if (autoBackup !== undefined)
            updateData.auto_backup = autoBackup;
...
app.post("/api/super-admin/settings/test-email", authMiddleware, superAdminMiddleware, async (req, res) => {
    try {
...
// 2. Recent Jobs
app.get("/api/home/recent-jobs", async (req, res) => {
    try {
...
app.get("/api/popular-searches", async (req, res) => {
    try {
        ...
        for (const job of jobTitles) {
            if (job.title?.trim())
                searches.push({
                    label: job.title.length > 25 ? job.title.slice(0, 22) + "..." : job.title,
