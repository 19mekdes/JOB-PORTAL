...
export class WebCryptoExtendedProvider extends WebCryptoProvider {
    ...
    async decryptSymmetric(data, key, algorithm) {
        if ((algorithm.type === 'AES_128_CBC' ||
            ...
            algorithm.type === 'SHA1_3DES_3KEY_CBC') {
            const algo = getPbeAlgorithm(algorithm.type);
            return algo.decrypt(data, key, algorithm.params.salt, algorithm.params.iterationCount);
        }
    ...
    async encrypt(data, publicKeyInfo, algorithm) {
        if (algorithm.type === 'RSASSA_PKCS1_v1_5') {
            const publicKeyPem = forge.pki.publicKeyFromPem(publicKeyInfo.getPublicKey().toPem());
            const encrypted = publicKeyPem.encrypt(forge.util.binary.raw.encode(data));
            if (!encrypted) {
    ...
    async decrypt(data, privateKeyInfo, algorithm) {
        ...
        if (algorithm.type === 'RSASSA_PKCS1_v1_5') {
            const privateKeyPem = forge.pki.privateKeyFromPem(privateKeyInfo.getPrivateKey().toPem());
            const decrypted = privateKeyPem.decrypt(forge.util.binary.raw.encode(data));
            if (!decrypted) {
