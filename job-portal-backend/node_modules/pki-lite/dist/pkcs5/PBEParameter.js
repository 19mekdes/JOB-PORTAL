...
import { PkiBase, asn1js } from '../core/PkiBase.js';
/**
 ...
 * ```
 */
export class PBEParameter extends PkiBase {
    salt;
    iterationCount;
    constructor(options) {
        super();
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Error('Invalid ASN.1 structure: expected Sequence but got ' +
                asn1.constructor.name);
        }
        const values = asn1.valueBlock.value;
        if (values.length !== 2) {
            throw new Error(`Invalid ASN.1 structure: expected 2 elements, got ${values.length}`);
        }
        const saltAsn1 = values[0];
        const iterationCountAsn1 = values[1];
        if (!(saltAsn1 instanceof asn1js.OctetString)) {
            throw new Error('Invalid ASN.1 structure: expected OctetString for salt');
        }
        if (!(iterationCountAsn1 instanceof asn1js.Integer)) {
            throw new Error('Invalid ASN.1 structure: expected Integer for iterationCount');
        }
