 ...
 * ```
 */
export class OCSPResponseBuilder {
    responderID;
    privateKey;
    producedAt;
    responseData = [];
     ...
     * @returns This builder for chaining
     */
    setResponderFromCertificate(certificate) {
    ...
    addCertificateResponse(options) {
        ...
        if (options.certificate && !options.issuerCertificate) {
            throw new Error('issuerCertificate is required when using Certificate objects');
        }
     ...
     */
    async createCertID(issuerCertificate, subjectCertificate) {
    ...
    async buildSingleResponses() {
        ...
        for (const data of this.responseData) {
            ...
            if (data.certID) {
                ...
                certID = await this.createCertID(data.issuerCertificate, data.certificate);
            }
            else {
                throw new Error('Invalid response data: missing certID and certificate');
            }
            // Create cert status
            const certStatus = data.status === 'good'
                ? CertStatus.createGood()
                : data.status === 'revoked'
                    ? CertStatus.createRevoked(data.revocationTime ?? new Date(), data.revocationReason)
                    : CertStatus.createUnknown();
            const thisUpdate = data.thisUpdate ?? new Date();
            responses.push(new SingleResponse({
                certID,
                certStatus,
                thisUpdate,
     ...
     * ```
     */
    addResponse(issuerOrCertID, certificateOrStatus, statusOrOptions, options) {
        // Pattern 1: addResponse(issuerCert, subjectCert, status, options?)
        if (issuerOrCertID instanceof Certificate &&
            certificateOrStatus instanceof Certificate) {
            const status = statusOrOptions;
            if (!status) {
                throw new Error('Status is required when using Certificate objects');
            }
        ...
        // Pattern 2: addResponse(certID, status, options?)
            ...
            const status = certificateOrStatus;
            ...
            throw new Error('Invalid arguments: expected (issuerCert, subjectCert, status, options?) or (certID, status, options?)');
        }
    }
    /**
     ...
     * @returns This builder for chaining
     */
    addResponseExtension(extension) {
    ...
    validate() {
        ...
        const statusValue = this.responseStatus instanceof OCSPResponseStatus
    ...
    async build() {
        ...
        const signatureAlgorithm = this.algorithm instanceof SignatureAlgorithmIdentifier
            ? this.algorithm
            : AlgorithmIdentifier.signatureAlgorithm(this.algorithm ?? {
        ...
        const tbsResponseData = new ResponseData({
            ...
            responseExtensions: this.responseExtensions.length > 0
                ? this.responseExtensions
                : undefined,
        });
        const signature = await signatureAlgorithm.sign(tbsResponseData, this.privateKey);
