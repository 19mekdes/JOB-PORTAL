...
export class PFXBuilder {
    ...
    validate() {
        ...
        if (this.certificates.length === 0 && this.privateKeys.length === 0) {
            throw new Error('At least one certificate or private key is required');
        }
    }
    /**
    ...
    async build() {
        ...
        for (const privateKey of this.privateKeys) {
            const encryptedKey = await EncryptedPrivateKeyInfo.create(privateKey, password, { iterations });
            keyBags.push(new SafeBag({
        ...
        if (keyBags.length > 0) {
            ...
            contentInfos.push(new ContentInfo({
        ...
        const macData = await MacData.create(authSafeDer, password, iterations, 'SHA-256');
        return new PFX({
