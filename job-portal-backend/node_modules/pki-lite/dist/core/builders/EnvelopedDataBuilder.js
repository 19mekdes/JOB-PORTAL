 ...
 */
export class EnvelopedDataBuilder {
    /**
     ...
     */
    static DEFAULT_KEY_ENCRYPTION_ALGORITHM = new KeyEncryptionAlgorithmIdentifier({
     ...
     * ```
     */
    setContentEncryptionAlgorithm(algorithm) {
     ...
     */
    getVersion(recipients) {
        if (this.originatorInfo) {
            return 2;
        }
        if (recipients.some((r) => r instanceof OtherRecipientInfo || r.version !== 0)) {
            return 2;
        }
        return 0;
    }
    ...
    async build() {
        ...
        const contentEncryptionAlgorithm = this.contentEncryptionAlgorithm instanceof AlgorithmIdentifier
            ? this
                .contentEncryptionAlgorithm
            : AlgorithmIdentifier.contentEncryptionAlgorithm(this.contentEncryptionAlgorithm);
        const contentEncryptionKey = contentEncryptionAlgorithm.generateKey();
        const encryptedContent = await contentEncryptionAlgorithm.encrypt(data, contentEncryptionKey);
        const encryptedContentInfo = new EncryptedContentInfo({
            contentType: this.contentType,
            contentEncryptionAlgorithm,
            encryptedContent: encryptedContent,
        });
        const recipientInfos = await Promise.all(this.recipients.map(async (recipient) => {
            const keyEncryptionAlgorithm = recipient.keyEncryptionAlgorithm
                ? recipient.keyEncryptionAlgorithm instanceof
                    AlgorithmIdentifier
                    ? recipient.keyEncryptionAlgorithm
                    : AlgorithmIdentifier.keyEncryptionAlgorithm(recipient.keyEncryptionAlgorithm)
            ...
            const issuerAndSerialNumber = new IssuerAndSerialNumber({
                ...
                serialNumber: recipient.certificate.tbsCertificate.serialNumber,
            });
            const publicKey = recipient.certificate.tbsCertificate.subjectPublicKeyInfo;
            const encryptedKey = await keyEncryptionAlgorithm.encrypt(contentEncryptionKey, publicKey);
