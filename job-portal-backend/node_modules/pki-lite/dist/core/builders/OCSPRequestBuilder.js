...
export class OCSPRequestBuilder {
     ...
     */
    async build() {
        const hashAlgorithm = AlgorithmIdentifier.digestAlgorithm(this.hashAlgorithm);
        const builtRequests = [...this.requests];
        for (const entry of this.pendingCertificates) {
            const { certificate, issuerCertificate, singleRequestExtensions } = entry;
            const certID = new CertID({
                hashAlgorithm,
                issuerNameHash: await hashAlgorithm.digest(issuerCertificate.tbsCertificate.subject.toDer()),
                issuerKeyHash: await hashAlgorithm.digest(issuerCertificate.getSubjectPublicKeyInfo()
                    .subjectPublicKey),
                serialNumber: certificate.tbsCertificate.serialNumber,
            });
            builtRequests.push(new Request({
                reqCert: certID,
        ...
        const tbsRequest = new TBSRequest({
            ...
            requestExtensions: this.requestExtensions.length > 0
