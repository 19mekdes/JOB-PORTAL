...
export class CertificateBuilder {
    ...
    extensions = [];
    algorithm;
    version = 2; // Default to v3 (version 2)
    /**
     ...
     * ```
     */
    addBasicConstraints(cA, pathLenConstraint) {
        this.extensions.push(Extension.basicConstraints({ cA, pathLenConstraint }));
        return this;
    }
    /**
     ...
     * ```
     */
    addSubjectAltName(...altNames) {
        const generalNames = altNames.map((name) => typeof name === 'string'
     ...
     * @returns This builder for chaining
     */
    addSubjectKeyIdentifier(keyIdentifier) {
     ...
     * @returns This builder for chaining
     */
    addAuthorityKeyIdentifier(keyIdentifier) {
    ...
    async selfSign() {
        this.validate();
        if (!this.privateKey) {
            throw new Error('Private key is required for self-signed certificate');
        }
        ...
        const issuerName = this.issuer instanceof Certificate
        ...
        const signatureValue = await signatureAlgorithm.sign(tbs.toDer(), this.privateKey);
    ...
    async sign() {
        ...
        const signatureValue = await signatureAlgorithm.sign(tbs.toDer(), signingKey);
        return new Certificate({
            tbsCertificate: tbs,
            signatureAlgorithm,
            signatureValue,
