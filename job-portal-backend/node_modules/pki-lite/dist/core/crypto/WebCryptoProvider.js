import { AlgorithmIdentifier, PSourceAlgorithm, } from '../../algorithms/AlgorithmIdentifier.js';
...
export class WebCryptoProvider {
    ...
    async digest(data, algorithm) {
        if (algorithm === 'MD5') {
            throw new UnsupportedCryptoAlgorithmError('MD5 is not supported in WebCrypto');
        }
     ...
     * @throws UnsupportedCryptoAlgorithmError if the algorithm is not supported
     */
    async hmac(key, data, hash) {
        if (hash === 'MD5') {
            throw new UnsupportedCryptoAlgorithmError('MD5 is not supported in WebCrypto');
        }
        const cryptoKey = await this.crypto.subtle.importKey('raw', key, { name: 'HMAC', hash: { name: hash } }, false, ['sign']);
    ...
    getWebCryptoAlgorithm(algorithm) {
        switch (algorithm.type) {
            ...
            default:
                const _exhaustiveCheck = algorithm;
                throw new UnsupportedCryptoAlgorithmError(`Unsupported algorithm type: ${_exhaustiveCheck}`);
        }
    }
    async sign(data, privateKeyInfo, algorithm) {
        const webCryptoParams = this.getWebCryptoAlgorithm(algorithm);
        const importedRsaKey = await this.crypto.subtle.importKey('pkcs8', privateKeyInfo.toDer(), webCryptoParams, false, ['sign']);
        const signature = await this.crypto.subtle.sign(webCryptoParams, importedRsaKey, data);
        return new Uint8Array(signature);
    }
    async verify(data, publicKeyInfo, signature, algorithm) {
        const webCryptoParams = this.getWebCryptoAlgorithm(algorithm);
        if (!webCryptoParams) {
            throw new UnsupportedCryptoAlgorithmError(`Unsupported algorithm: ${algorithm.type}`);
        }
        const importedKey = await this.crypto.subtle.importKey('spki', publicKeyInfo.toDer(), webCryptoParams, false, ['verify']);
        const valid = await this.crypto.subtle.verify(webCryptoParams, importedKey, signature, data);
        return valid;
    }
    getWebCryptoSymmetricAlgorithm(algorithm) {
        switch (algorithm.type) {
            case 'AES_128_CBC':
            case 'AES_192_CBC':
            case 'AES_256_CBC': {
                if (algorithm.params.disablePadding) {
                    throw new UnsupportedCryptoAlgorithmError('AES-CBC with no padding is not supported in WebCrypto');
                }
                const iv = algorithm.params.nonce;
                if (!iv) {
                    throw new UnsupportedCryptoAlgorithmError(`Invalid IV for AES-CBC: ${algorithm.params}`);
                }
            ...
            case 'AES_256_GCM': {
                ...
                if (!gcmParams.nonce) {
                    throw new UnsupportedCryptoAlgorithmError(`Invalid IV for AES-GCM: ${algorithm.params}`);
                }
                return {
                    name: 'AES-GCM',
                    iv: gcmParams.nonce,
            ...
            case 'AES_256_CCM': {
                ...
                if (!ccmParams.nonce) {
                    throw new UnsupportedCryptoAlgorithmError(`Invalid nonce for AES-CCM: ${algorithm.params}`);
                }
                return {
                    name: 'AES-CCM',
                    iv: ccmParams.nonce,
            ...
            case 'AES_192_ECB':
            case 'AES_256_ECB':
                throw new UnsupportedCryptoAlgorithmError('ECB mode is not supported by WebCryptoProvider. Use an extended crypto provider that supports legacy algorithms.');
            case 'PBES2':
                throw new UnsupportedCryptoAlgorithmError('PBES2 is not supported for symmetric encryption in WebCrypto. Use an extended crypto provider that supports legacy algorithms.');
            ...
            case 'PKCS12_SHA1_RC4_40':
                throw new UnsupportedCryptoAlgorithmError(`PKCS#12 PBE algorithm ${algorithm.type} is not supported by WebCryptoProvider. Use an extended crypto provider that supports legacy algorithms.`);
            default:
                const _exhaustiveCheck1 = algorithm;
                throw new UnsupportedCryptoAlgorithmError(`Unsupported symmetric encryption algorithm: ${_exhaustiveCheck1}`);
        }
    }
    getWebCryptoSymmetricKeyGenAlgorithm(algorithm) {
        let name;
        let length = 128;
        if (algorithm.includes('128')) {
            ...
            throw new UnsupportedCryptoAlgorithmError(`Unsupported symmetric encryption algorithm: ${algorithm}`);
        }
        switch (algorithm) {
            case 'AES_128_CBC':
            case 'AES_192_CBC':
            ...
            case 'AES_256_ECB':
                throw new UnsupportedCryptoAlgorithmError('ECB mode is not supported by WebCryptoProvider. Use an extended crypto provider that supports legacy algorithms.');
            case 'PBES2':
                throw new UnsupportedCryptoAlgorithmError('PBES2 is not supported for key generation in WebCryptoProvider. Use an extended crypto provider that supports legacy algorithms.');
            case 'PKCS12_SHA1_RC4_128':
            case 'PKCS12_SHA1_RC4_40':
            case 'PKCS12_SHA1_3DES_3KEY':
            case 'PKCS12_SHA1_3DES_2KEY':
            case 'PKCS12_SHA1_RC2_128':
            case 'PKCS12_SHA1_RC2_40':
                throw new UnsupportedCryptoAlgorithmError(`PKCS#12 PBE algorithm ${algorithm} is not supported by WebCryptoProvider`);
            default:
                const _exhaustiveCheck2 = algorithm;
                throw new UnsupportedCryptoAlgorithmError(`Unsupported symmetric encryption algorithm: ${_exhaustiveCheck2}`);
        }
        return { name, length };
    }
    getWebCryptoDerivationAlgorithm(algorithm) {
        switch (algorithm.type) {
            case 'PBKDF2':
                return {
                    name: 'PBKDF2',
            ...
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported key derivation algorithm: ${algorithm.type}`);
        }
    }
    ...
    async getKeyMaterial(password) {
        ...
        const keyMaterial = await this.crypto.subtle.importKey('raw', password, { name: 'PBKDF2' }, false, ['deriveBits', 'deriveKey']);
        return keyMaterial;
    }
    async deriveCryptoKey(password, algorithm) {
        ...
        const webCryptoDerivationAlgorithm = this.getWebCryptoDerivationAlgorithm(derivationAlgorithm);
        const encryptionAlgorithmParams = this.getWebCryptoSymmetricKeyGenAlgorithm(algorithm.params.encryptionAlgorithm.type);
        const derivedKey = await this.crypto.subtle.deriveKey(webCryptoDerivationAlgorithm, keyMaterial, encryptionAlgorithmParams, false, ['encrypt', 'decrypt']);
        return derivedKey;
    }
    /**
     ...
     * ```
     */
    async derivePkcs12Key(password, salt, iterationCount, keyLength, purpose = 'encryption', hash = 'SHA-1') {
        ...
        return pkcs12Derive(passwordBytes, salt, id, iterationCount, keyLength, hash);
    }
    async deriveKey(password, algorithm) {
        // Handle PKCS#12 PBE algorithms using derivePkcs12Key
        if (algorithm.type !== 'PBES2') {
            if (password instanceof CryptoKey) {
                throw new UnsupportedCryptoAlgorithmError(`PKCS#12 PBE algorithm ${algorithm.type} requires a password, not a CryptoKey`);
            }
            ...
            switch (algorithm.type) {
                ...
                default:
                    ...
                    throw new UnsupportedCryptoAlgorithmError(`Unsupported PKCS#12 PBE algorithm: ${algorithm.type}`);
            }
            // Legacy PKCS#12 algorithms always use SHA-1
            return this.derivePkcs12Key(password, salt, iterationCount, keyLength, 'encryption', 'SHA-1');
        }
    ...
    async encryptSymmetric(data, key, algorithm) {
        if (algorithm.type === 'PBES2') {
            const derivedKey = await this.deriveCryptoKey(key, algorithm);
            return this.encryptSymmetric(data, derivedKey, algorithm.params.encryptionAlgorithm);
        }
        else {
            ...
            const cryptoKey = key instanceof CryptoKey
                ? key
                : await this.crypto.subtle.importKey('raw', typeof key === 'string'
                    ? new TextEncoder().encode(key)
                    : key, params, false, ['encrypt']);
            return new Uint8Array(await this.crypto.subtle.encrypt(params, cryptoKey, data));
        }
    }
    async encrypt(data, publicKeyInfo, algorithm) {
        const params = this.getWebCryptoAlgorithm(algorithm);
        if (!params)
            throw new UnsupportedCryptoAlgorithmError(`Unsupported asymmetric encryption algorithm: ${algorithm}`);
        const cryptoKey = await this.crypto.subtle.importKey('spki', publicKeyInfo.toDer(), params, false, ['encrypt']);
        return new Uint8Array(await this.crypto.subtle.encrypt(params, cryptoKey, data));
    }
    async decrypt(data, privateKeyInfo, algorithm) {
        const params = this.getWebCryptoAlgorithm(algorithm);
        const cryptoKey = await this.crypto.subtle.importKey('pkcs8', privateKeyInfo.toDer(), params, false, ['decrypt']);
        return new Uint8Array(await this.crypto.subtle.decrypt(params, cryptoKey, data));
    }
    async decryptSymmetric(data, key, algorithm) {
        if (algorithm.type === 'PBES2') {
            const derivedKey = await this.deriveCryptoKey(key, algorithm);
            return this.decryptSymmetric(data, derivedKey, algorithm.params.encryptionAlgorithm);
        }
        else {
            const params = this.getWebCryptoSymmetricAlgorithm(algorithm);
            if (!params)
                throw new UnsupportedCryptoAlgorithmError(`Unsupported symmetric decryption algorithm: ${algorithm}`);
            const cryptoKey = key instanceof CryptoKey
                ? key
                : await this.crypto.subtle.importKey('raw', typeof key === 'string'
                    ? new TextEncoder().encode(key)
                    : key, params, false, ['decrypt']);
            return new Uint8Array(await this.crypto.subtle.decrypt(params, cryptoKey, data));
        }
    }
    ...
    async generateKeyPair(options) {
        ...
        if (options.algorithm === 'RSA') {
            const modulusLength = typeof options.keySize === 'number' ? options.keySize : 2048;
            webCryptoAlgorithm = {
                name: 'RSASSA-PKCS1-v1_5',
                modulusLength,
                publicExponent: options.publicExponent ?? new Uint8Array([1, 0, 1]), // 65537
                hash: { name: options.hash ?? 'SHA-256' },
            };
        }
        else if (options.algorithm === 'EC') {
            const namedCurve = typeof options.namedCurve === 'string'
                ? options.namedCurve
                : 'P-256';
            webCryptoAlgorithm = {
                name: 'ECDSA',
            ...
            throw new UnsupportedCryptoAlgorithmError(`Unsupported algorithm: ${options.algorithm}`);
        }
        const keyPair = (await this.crypto.subtle.generateKey(webCryptoAlgorithm, true, ['sign', 'verify']));
        const privateKey = await this.crypto.subtle.exportKey('pkcs8', keyPair.privateKey);
        const publicKey = await this.crypto.subtle.exportKey('spki', keyPair.publicKey);
        return {
            privateKey: new Uint8Array(privateKey),
            publicKey: new Uint8Array(publicKey),
        };
    }
    getEcCurveParameters(encryptionParams) {
        if (encryptionParams.type === 'ECDSA') {
            switch (encryptionParams.params.namedCurve) {
                ...
                default:
                    throw new UnsupportedCryptoAlgorithmError(`Unsupported named curve: ${encryptionParams.params.namedCurve}`);
            }
        }
        throw new UnsupportedCryptoAlgorithmError(`Unsupported encryption parameters: ${JSON.stringify(encryptionParams)}`);
    }
    signatureAlgorithm(encryptionParams) {
        if (encryptionParams.type === 'ECDH') {
            throw new Error(`ECDH is not a signature algorithm`);
        }
        const digestAlgorithm = this.digestAlgorithm(encryptionParams.params.hash);
        if (encryptionParams.type === 'RSA_OAEP') {
            ...
            let pSourceAlgorithm = undefined;
        ...
        const key = `${encryptionParams.type}/${encryptionParams.params.hash}`;
        switch (key) {
            ...
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported signature algorithm: ${key}`);
        }
    }
    digestAlgorithm(hash) {
        switch (hash) {
            ...
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported hash algorithm: ${hash}`);
        }
    }
    keyEncryptionAlgorithm(encryptionParams) {
        switch (encryptionParams.type) {
            case 'ECDH':
                return new AlgorithmIdentifier({
                    algorithm: OIDs.EC.ECDH,
                    parameters: AlgorithmIdentifier.getEcCurveParameters(encryptionParams),
                });
            default:
                return this.signatureAlgorithm(encryptionParams);
        }
    }
    contentEncryptionAlgorithm(encryptionParams) {
        let algorithm;
        let parameters;
        switch (encryptionParams.type) {
            case 'AES_128_CBC':
                    ...
                    keyDerivationFunc: new AlgorithmIdentifier({
                        ...
                        parameters: new PBKDF2Params({
                            salt: encryptionParams.params.derivationAlgorithm
                                .params.salt,
                            iterationCount: encryptionParams.params.derivationAlgorithm
                                .params.iterationCount,
                            keyLength: encryptionParams.params.derivationAlgorithm
                                .params.keyLength,
                            prf: encryptionParams.params.derivationAlgorithm
                                .params.hash
                                ? this.digestAlgorithm(encryptionParams.params
                                    .derivationAlgorithm.params.hash)
                                : undefined,
                        }),
                    }),
                    encryptionScheme: this.contentEncryptionAlgorithm(encryptionParams.params.encryptionAlgorithm),
                });
                break;
            case 'PKCS12_SHA1_3DES_2KEY':
            case 'PKCS12_SHA1_3DES_3KEY':
            ...
            case 'PKCS12_SHA1_RC4_128':
            case 'PKCS12_SHA1_RC4_40':
                throw new UnsupportedCryptoAlgorithmError(`PKCS#12 PBE algorithm ${encryptionParams.type} is not supported by WebCryptoProvider. Use an extended crypto provider that supports legacy algorithms.`);
            default:
                const _exhaustiveCheck = encryptionParams;
                throw new UnsupportedCryptoAlgorithmError(`Unsupported symmetric encryption algorithm: ${_exhaustiveCheck}. Use an extended crypto provider that supports legacy algorithms.`);
        }
    ...
    toDerivationAlgorithmParams(algorithm) {
        switch (algorithm.algorithm.toString()) {
            case OIDs.PKCS5.PBKDF2: {
                const params = algorithm.parameters?.parseAs(PBKDF2Params);
                if (!params) {
                    throw new Error(`Invalid PBKDF2 parameters: ${algorithm.parameters}`);
                }
                if (!(params.salt instanceof OctetString)) {
                    throw new Error(`Unsupported PBKDF2 salt type: ${params.salt}`);
                }
                return {
                    type: 'PBKDF2',
                    params: {
            ...
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported key derivation algorithm: ${algorithm.friendlyName}`);
        }
    }
    toPbeAlgorithmParams(algorithm) {
        ...
        switch (oidString) {
            case OIDs.PKCS5.PBES2: {
                ...
                if (!parameters) {
                    throw new Error(`Invalid PBES2 parameters: ${algorithm.parameters}`);
                }
                return {
                    type: 'PBES2',
                    params: {
                        derivationAlgorithm: this.toDerivationAlgorithmParams(parameters.keyDerivationFunc),
                        encryptionAlgorithm: this.toSymmetricEncryptionAlgorithmParams(parameters.encryptionScheme),
                    },
                };
            }
            ...
            case OIDs.PKCS12.PBE.SHA1_RC4_128: {
                ...
                if (!parameters) {
                    throw new Error(`Invalid PKCS#12 PBE parameters: ${algorithm.parameters}`);
                }
                return {
            ...
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported PBE algorithm: ${algorithm.friendlyName}`);
        }
    }
    toSymmetricEncryptionAlgorithmParams(algorithm) {
        let type;
        switch (algorithm.algorithm.toString()) {
            ...
            case OIDs.ENCRYPTION.AES_256_CBC: {
                ...
                if (!iv) {
                    throw new Error(`Invalid IV for AES-CBC: ${algorithm.parameters}`);
                }
                return {
                    type,
            ...
            case OIDs.ENCRYPTION.AES_256_GCM: {
                ...
                if (!gcmParams) {
                    throw new Error(`Invalid GCM parameters for AES-GCM: ${algorithm.parameters}`);
                }
            ...
            case OIDs.ENCRYPTION.AES_256_CCM: {
                ...
                if (!ccmParams) {
                    throw new Error(`Invalid CCM parameters for AES-CCM: ${algorithm.parameters}`);
                }
            ...
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported symmetric encryption algorithm: ${algorithm.friendlyName}`);
        }
    }
    toAsymmetricEncryptionAlgorithmParams(algorithm, publicKeyInfo) {
        ...
        switch (algorithm.algorithm.value) {
            case OIDs.RSA.RSASSA_PSS:
                ...
                const rassParams = RSASSAPSSParams.fromAsn1(algorithm.parameters.toAsn1());
                return {
                    type: 'RSA_PSS',
                    params: {
                        hash: this.toHashAlgorithm(rassParams.getEffectiveHashAlgorithm()),
            ...
            case OIDs.RSA.RSAES_OAEP:
                ...
                const oaepParams = RSAESOAEPParams.fromAsn1(algorithm.parameters.toAsn1());
                const labelBytes = oaepParams.pSourceAlgorithm?.getLabelBytes();
                return {
                    type: 'RSA_OAEP',
                    params: {
                        hash: this.toHashAlgorithm(oaepParams.getEffectiveHashAlgorithm()),
                        ...(labelBytes ? { label: labelBytes } : {}),
                        pSourceAlgorithm: oaepParams.pSourceAlgorithm?.algorithm.value,
                    },
                };
            ...
            case OIDs.EC.SHA1_WITH_ECDSA:
                return {
                    ...
                    params: {
                        hash: 'SHA-1',
                        namedCurve: this.getEcNamedCurve(algorithm, publicKeyInfo),
                    },
                };
            ...
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported algorithm: ${algorithm.friendlyName}`);
        }
    }
    getEcNamedCurve(algorithm, publicKeyInfo) {
        let curve = algorithm.parameters?.parseAs(ObjectIdentifier)?.value ??
        ...
        switch (curve) {
            ...
            case OIDs.CURVES.SECP521R1:
                return 'P-521';
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported curve OID: ${curve}`);
        }
    }
    toHashAlgorithm(algorithm) {
        switch (algorithm.algorithm.value) {
            ...
            case OIDs.HASH.HMAC_SHA512:
                return 'SHA-512';
            default:
                throw new UnsupportedCryptoAlgorithmError(`Unsupported hash algorithm: ${algorithm.friendlyName}`);
        }
    }
}
