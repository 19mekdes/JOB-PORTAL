 ...
 * ```
 */
export class CCMParameters extends PkiBase {
    aesNonce;
    aesICVlen;
    /**
    ...
    constructor(options) {
        ...
        // Validate nonce size
        if (nonceOctetString.bytes.length < 7 ||
        ...
        if (!validLengths.includes(aesICVlen)) {
            throw new Error('AES-CCM ICV length must be one of: 4, 6, 8, 10, 12, 14, or 16');
        }
    ...
    static fromAsn1(asn1) {
        ...
        if (sequenceValue.length < 1 || sequenceValue.length > 2) {
            throw new Asn1ParseError('CCMParameters sequence must have 1 or 2 elements');
        }
