 ...
 * ```
 */
export class GCMParameters extends PkiBase {
    aesNonce;
    aesICVlen;
    /**
    ...
    constructor(options) {
        ...
        if (!validLengths.includes(aesICVlen)) {
            throw new Error('AES-GCM ICV length must be one of: 12, 13, 14, 15, or 16');
        }
    ...
    static fromAsn1(asn1) {
        ...
        if (sequenceValue.length < 1 || sequenceValue.length > 2) {
            throw new Asn1ParseError('GCMParameters sequence must have 1 or 2 elements');
        }
        const aesNonce = new Uint8Array(sequenceValue[0].valueBlock.valueHexView);
        let aesICVlen;
