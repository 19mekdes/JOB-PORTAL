 ...
 * ```
 */
export class BMPString extends PkiBase {
    bytes;
    constructor(options) {
        super();
        const { value } = options;
     ...
     */
    static nullTerminated(password) {
        const str = typeof password === 'string'
            ? password
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.BmpString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected BMPString');
        }
        // Get the decoded string value from the ASN.1 structure
        const value = asn1.valueBlock.value;
        if (!value) {
            throw new Asn1ParseError('Could not extract value from ASN.1 BMPString');
        }
