import { asn1js, PkiBase } from '../core/PkiBase.js';
import { Asn1ParseError } from '../core/errors/Asn1ParseError.js';
/**
 * Represents an ASN.1 UTCTime value.
 *
 * @asn
 * ```asn
 * UTCTime ::= <value>
 * ```
 */
export class UTCTime extends PkiBase {
    time;
    constructor(options = {}) {
        super();
        this.time = options.time ? new Date(options.time) : new Date();
    }
    toAsn1() {
        return new asn1js.UTCTime({ valueDate: this.time });
    }
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.UTCTime)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected UTCTime');
        }
        return new UTCTime({ time: asn1.toDate() });
    }
}
export class GeneralizedTime extends PkiBase {
    date;
    constructor(options = {}) {
        super();
        this.date = options.date ? new Date(options.date) : new Date();
    }
    toAsn1() {
        return new asn1js.GeneralizedTime({ valueDate: this.date });
    }
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.GeneralizedTime)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected GeneralizedTime');
        }
        return new GeneralizedTime({ date: asn1.toDate() });
    }
}
