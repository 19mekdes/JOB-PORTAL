 ...
 * ```
 */
export class PasswordRecipientInfo extends PkiBase {
    version;
    ...
    constructor(options) {
        super();
        const { version, keyEncryptionAlgorithm, encryptedKey, keyDerivationAlgorithm, } = options;
    ...
    toAsn1() {
        ...
        values.push(this.keyEncryptionAlgorithm.toAsn1(), new asn1js.OctetString({ valueHex: this.encryptedKey }));
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected Sequence for PasswordRecipientInfo');
        }
        const values = asn1.valueBlock.value;
        if (values.length < 3 || values.length > 4) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected 3-4 elements, got ${values.length}`);
        }
        // Check version
        ...
        if (version !== CMSVersion.v0) {
            throw new Asn1ParseError(`Invalid version: expected 0, got ${version}`);
        }
        let currentIndex = 1;
        let keyDerivationAlgorithm = undefined;
        ...
        if (values.length > 3 &&
            ...
            const constructed = values[currentIndex];
        ...
        if (!(values[currentIndex] instanceof asn1js.OctetString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected OctetString for encryptedKey');
        }
        const octetString = values[currentIndex];
