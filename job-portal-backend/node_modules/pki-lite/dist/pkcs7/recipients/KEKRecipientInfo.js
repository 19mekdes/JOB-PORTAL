 ...
 * ```
 */
export class KEKRecipientInfo extends PkiBase {
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected Sequence for KEKRecipientInfo');
        }
        const values = asn1.valueBlock.value;
        if (values.length !== 4) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected 4 elements, got ${values.length}`);
        }
        // Check version
        ...
        if (version !== CMSVersion.v4) {
            throw new Asn1ParseError(`Invalid version: expected 4, got ${version}`);
        }
        // Extract kekid
