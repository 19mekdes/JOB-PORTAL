 ...
 */
export class RecipientEncryptedKeys extends PkiSet {
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected Sequence for RecipientEncryptedKeys');
        }
        const recipientEncryptedKeys = asn1.valueBlock.value.map((keyAsn1) => RecipientEncryptedKey.fromAsn1(keyAsn1));
 ...
 * ```
 */
export class KeyAgreeRecipientInfo extends PkiBase {
    version;
    originator;
    ukm;
    ...
    constructor(options) {
        super();
        const { version, originator, keyEncryptionAlgorithm, recipientEncryptedKeys, ukm, } = options;
    ...
    toAsn1() {
        ...
        // Add ukm if present
        if (this.ukm) {
            values.push(new asn1js.Constructed({
                idBlock: {
                    tagClass: 3, // CONTEXT_SPECIFIC
                    tagNumber: 1, // [1]
                },
        ...
        values.push(this.keyEncryptionAlgorithm.toAsn1(), this.recipientEncryptedKeys.toAsn1());
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected Sequence for KeyAgreeRecipientInfo');
        }
        const values = asn1.valueBlock.value;
        if (values.length < 4 || values.length > 5) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected 4-5 elements, got ${values.length}`);
        }
        // Check version
        ...
        if (version !== 3) {
            throw new Asn1ParseError(`Invalid version: expected 3, got ${version}`);
        }
        // Extract originator
        if (!(values[1] instanceof asn1js.Constructed) ||
            values[1].idBlock.tagClass !== 3 ||
            values[1].idBlock.tagNumber !== 0) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected [0] for originator');
        }
        ...
        if (values.length > 4 &&
            ...
            const ukmAsn1 = values[currentIndex]
                .valueBlock.value[0];
            if (!(ukmAsn1 instanceof asn1js.OctetString)) {
                throw new Asn1ParseError('Invalid ASN.1 structure: expected OctetString for ukm');
            }
            ukm = new Uint8Array(ukmAsn1.valueBlock.valueHexView);
            currentIndex++;
        }
        // Extract keyEncryptionAlgorithm
        const keyEncryptionAlgorithm = AlgorithmIdentifier.fromAsn1(values[currentIndex]);
        currentIndex++;
        // Extract recipientEncryptedKeys
        const recipientEncryptedKeys = RecipientEncryptedKeys.fromAsn1(values[currentIndex]);
