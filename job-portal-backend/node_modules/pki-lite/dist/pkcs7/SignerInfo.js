 ...
 * ```
 */
export class SignerInfo extends PkiBase {
    ...
    constructor(options) {
        super();
        const { version, sid, digestAlgorithm, signatureAlgorithm, signature, signedAttrs, unsignedAttrs, } = options;
    ...
    toAsn1() {
        ...
        values.push(this.signatureAlgorithm.toAsn1(), new asn1js.OctetString({ valueHex: this.signature }));
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 5) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected at least 5 elements, got ${values.length}`);
        }
        let index = 0;
        // Version
        if (!(values[index] instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected Integer for version');
        }
        const version = values[index].valueBlock.valueDec;
        index++;
        ...
        if (!(values[index] instanceof asn1js.OctetString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected OctetString for signature');
        }
        const signature = new Uint8Array(values[index].valueBlock.valueHexView);
        index++;
        // UnsignedAttrs (optional)
        let unsignedAttrs = undefined;
        if (index < values.length &&
            values[index].idBlock.tagClass === 3 && // CONTEXT-SPECIFIC
    ...
    getSignatureData(data) {
        ...
        throw new Error('No signed attributes present and no data provided for signature verification');
    }
    async verify(options) {
        ...
        try {
            ...
            if (this.signedAttrs) {
                const messageDigestAttr = this.signedAttrs.find((attr) => attr.type.value === OIDs.PKCS9.MESSAGE_DIGEST);
                if (!messageDigestAttr) {
                    reasons.push('Signed attributes present but message digest attribute is missing');
                ...
                const messageDigestOctetString = messageDigestAttr.values[0].parseAs(OctetString);
                const expectedDigest = messageDigestOctetString.bytes;
                // Compute the actual digest of the content
                const actualDigest = await this.digestAlgorithm.digest(options.data);
                // Compare the digests
                if (expectedDigest.length !== actualDigest.length ||
                    !expectedDigest.every((byte, index) => byte === actualDigest[index])) {
                    reasons.push('Message digest in signed attributes does not match computed digest of content');
                    return { valid: false, reasons };
                }
            }
            const valid = await this.signatureAlgorithm.verify(this.getSignatureData(options.data), this.signature, options.publicKeyInfo);
            if (valid) {
                return { valid: true };
            }
            reasons.push(`${this.sid.toHumanString()}: Signature verification failed`);
        }
        catch (error) {
            if (error instanceof Error) {
                reasons.push(`${this.sid.toHumanString()}: Verification failed: ${error.message}`);
            }
            else {
                reasons.push(`${this.sid.toHumanString()}: Verification failed: ${String(error)}`);
            }
        }
        return { valid: false, reasons };
    }
