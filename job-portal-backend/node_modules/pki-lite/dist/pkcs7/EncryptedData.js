 ...
 * ```
 */
export class EncryptedData extends PkiBase {
    version;
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length !== 2) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected 2 elements, got ${values.length}`);
        }
        // Version
    ...
    static async create(options) {
        ...
        if (options.salt)
            builder.setSalt(options.salt);
        if (options.iv)
            builder.setIV(options.iv);
        if (options.iterations)
            builder.setIterations(options.iterations);
