 ...
 * ```
 */
export class AuthEnvelopedData extends PkiBase {
    /**
    ...
    constructor(options) {
        super();
        const { version, recipientInfos, authEncryptedContentInfo, mac, originatorInfo, authAttrs, unauthAttrs, } = options;
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 4) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected at least 4 elements, got ${values.length}`);
        }
        let currentIndex = 0;
        ...
        if (currentIndex >= values.length) {
            throw new Asn1ParseError('Invalid ASN.1 structure: missing authEncryptedContentInfo');
        }
        const authEncryptedContentInfo = EncryptedContentInfo.fromAsn1(values[currentIndex]);
        currentIndex++;
