import { asn1js, derToAsn1, PkiBase, PkiSet, } from '../core/PkiBase.js';
...
import { OriginatorInfo, RevocationInfoChoices, } from './recipients/OriginatorInfo.js';
 ...
 * ```
 */
export class EnvelopedData extends PkiBase {
    /**
     * Represents a set of RecipientInfo structures.
     *
     * @asn
    ...
    constructor(options) {
        super();
        const { version, recipientInfos, encryptedContentInfo, originatorInfo, unprotectedAttrs, } = options;
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 3) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected at least 3 elements, got ${values.length}`);
        }
        let currentIndex = 0;
        ...
        if (currentIndex >= values.length) {
            throw new Asn1ParseError('Invalid ASN.1 structure: missing encryptedContentInfo');
        }
        const encryptedContentInfo = EncryptedContentInfo.fromAsn1(values[currentIndex]);
        currentIndex++;
        ...
        if (currentIndex < values.length &&
            ...
            const unprotectedAttrsAsn1 = values[currentIndex].valueBlock.value[0];
            unprotectedAttrs =
    ...
    static fromCms(cms) {
        ...
        if (cms.contentType.toString() !== OIDs.PKCS7.ENVELOPED_DATA) {
            throw new Asn1ParseError(`Invalid content type: expected ${OIDs.PKCS7.ENVELOPED_DATA}, got ${cms.contentType}`);
        }
        if (!cms.content) {
    ...
    async decrypt(privateKey) {
        ...
        for (const recipientInfo of this.recipientInfos) {
            try {
                if (recipientInfo instanceof KeyTransRecipientInfo) {
                    ...
                    decryptedKey =
                        await recipientInfo.keyEncryptionAlgorithm.decrypt(encryptedKey, privateKey);
                }
            }
            catch (e) {
        ...
        const originalContent = await this.encryptedContentInfo.decrypt(decryptedKey);
    ...
    addOcsp(OCSP) {
        ...
        this.originatorInfo.crls.push(new OtherRevocationInfoFormat({
