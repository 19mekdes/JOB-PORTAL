import { asn1js, PkiBase, PkiSet, derToAsn1, } from '../core/PkiBase.js';
 ...
 * ```
 */
class DigestAlgorithmIdentifiers extends PkiSet {
    constructor(...items) {
        super(...items.map((item) => new DigestAlgorithmIdentifier({
 ...
 * ```
 */
class SignerInfos extends PkiSet {
}
/**
 ...
 * ```
 */
export class SignedData extends PkiBase {
    /**
    ...
    constructor(options) {
        super();
        const { version, digestAlgorithms, encapContentInfo, signerInfos, certificates, crls, } = options;
        this.version = version;
        this.digestAlgorithms = new DigestAlgorithmIdentifiers(...digestAlgorithms);
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure for SignedData: expected SEQUENCE');
        }
        // Extract the components from the ASN.1 structure
        if (asn1.valueBlock.value.length < 4) {
            throw new Asn1ParseError('Invalid ASN.1 structure for SignedData: insufficient elements');
        }
        // Extract the version
        const versionAsn1 = asn1.valueBlock.value[0];
        if (!(versionAsn1 instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure for SignedData: expected INTEGER for version');
        }
        ...
        if (!(digestAlgorithmsAsn1 instanceof asn1js.Set)) {
            throw new Asn1ParseError('Invalid ASN.1 structure for SignedData: expected SET for digestAlgorithms');
        }
        const digestAlgorithms = [];
        for (const digestAlgorithmAsn1 of digestAlgorithmsAsn1.valueBlock
            .value) {
            digestAlgorithms.push(DigestAlgorithmIdentifier.fromAsn1(digestAlgorithmAsn1));
        }
        // Extract encapsulated content info
        const encapContentInfoAsn1 = asn1.valueBlock.value[2];
        const encapContentInfo = EncapsulatedContentInfo.fromAsn1(encapContentInfoAsn1);
        ...
        if (currentIndex < asn1.valueBlock.value.length &&
            ...
            asn1.valueBlock.value[currentIndex].idBlock
                .tagNumber === 0) {
            const certificatesAsn1 = asn1.valueBlock.value[currentIndex];
            certificates = [];
        ...
        if (currentIndex < asn1.valueBlock.value.length &&
            ...
            asn1.valueBlock.value[currentIndex].idBlock
                .tagNumber === 1) {
            const crlsAsn1 = asn1.valueBlock.value[currentIndex];
            crls = [];
        ...
        if (currentIndex >= asn1.valueBlock.value.length) {
            throw new Asn1ParseError('Invalid ASN.1 structure for SignedData: missing signerInfos');
        }
        const signerInfosAsn1 = asn1.valueBlock.value[currentIndex];
        if (!(signerInfosAsn1 instanceof asn1js.Set)) {
            throw new Asn1ParseError('Invalid ASN.1 structure for SignedData: expected SET for signerInfos');
        }
        const signerInfos = [];
    ...
    static fromCms(cms) {
        ...
        if (cms.contentType.toString() !== OIDs.PKCS7.SIGNED_DATA) {
            throw new Asn1ParseError(`Invalid content type: expected ${OIDs.PKCS7.SIGNED_DATA}, got ${cms.contentType}`);
        }
        if (!cms.content) {
    ...
    async verify(options) {
        ...
        if (options.certificateValidation) {
            ...
            const validationOptions = options.certificateValidation === true
                ? {
                    validateChain: true,
                    checkCRL: true,
                    checkOCSP: true,
                    otherCertificates: this.certificates?.filter((c) => c instanceof Certificate),
                    checkSignature: true,
                    enforceCAConstraints: true,
                    validateNameConstraints: true,
                }
                : options.certificateValidation;
            const validationResult = await certificateValidator.validate(this.findCertificateForSignerInfo(validSignerInfo), validationOptions);
    ...
    findCertificateForSignerInfo(signerInfo) {
