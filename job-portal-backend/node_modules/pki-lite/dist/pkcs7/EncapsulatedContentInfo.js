 ...
 * ```
 */
export class EncapsulatedContentInfo extends PkiBase {
    eContentType;
    eContent;
    constructor(options) {
        super();
        ...
        this.eContent = eContent
            ? new Uint8Array(typeof eContent === 'string'
     ...
     */
    toAsn1() {
        const values = [this.eContentType.toAsn1()];
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 1 || values.length > 2) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected 1-2 elements, got ${values.length}`);
        }
        // ContentType
        if (!(values[0] instanceof asn1js.ObjectIdentifier)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected ObjectIdentifier for contentType');
        }
        ...
        if (values.length > 1) {
            ...
            if (!(values[1] instanceof asn1js.Constructed) ||
                ...
                // [0]
                throw new Asn1ParseError('Invalid ASN.1 structure: expected [0] EXPLICIT tag for content');
            }
            const contentValues = values[1].valueBlock
                .value;
            if (contentValues.length !== 1) {
                throw new Asn1ParseError('Invalid ASN.1 structure: expected exactly one content element inside [0] tag');
            }
            if (!(contentValues[0] instanceof asn1js.OctetString)) {
                throw new Asn1ParseError('Invalid ASN.1 structure: expected OCTET STRING for eContent');
            }
