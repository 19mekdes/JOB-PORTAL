 ...
 * ```
 */
export class RevokedCertificate extends PkiBase {
    userCertificate;
    revocationDate;
    crlEntryExtensions;
    constructor(options) {
    ...
    toAsn1() {
        ...
        const serialValue = typeof this.userCertificate === 'string'
            ? parseInt(this.userCertificate, 16) // Parse as hex since certificate serial numbers are typically hex
            : this.userCertificate;
        values.push(new asn1js.Integer({
            value: serialValue,
        }));
        // Revocation Date
        values.push(new asn1js.UTCTime({
    ...
    static fromAsn1(asn1) {
        ...
        if (asn1.valueBlock.value.length < 2 ||
            ...
            throw new Asn1ParseError('Invalid ASN.1 structure: expected 2 or 3 elements');
        }
        const userCertificateAsn1 = asn1.valueBlock.value[0];
        const revocationDateAsn1 = asn1.valueBlock.value[1];
        const crlEntryExtensionsAsn1 = asn1.valueBlock.value.length > 2
            ? asn1.valueBlock.value[2]
            : undefined;
        if (!(userCertificateAsn1 instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: userCertificate must be an INTEGER');
        }
        ...
        if (revocationDateAsn1 instanceof asn1js.UTCTime) {
            ...
            throw new Asn1ParseError('Invalid ASN.1 structure: revocationDate must be a UTCTime or GeneralizedTime');
        }
        // Parse optional extensions
        let crlEntryExtensions;
        if (crlEntryExtensionsAsn1) {
            if (!(crlEntryExtensionsAsn1 instanceof asn1js.Sequence)) {
                throw new Asn1ParseError('Invalid ASN.1 structure: crlEntryExtensions must be a SEQUENCE');
            }
            crlEntryExtensions = crlEntryExtensionsAsn1.valueBlock.value.map((extensionAsn1) => {
                if (!(extensionAsn1 instanceof asn1js.Sequence)) {
                    throw new Asn1ParseError('Invalid ASN. ASN.1 structure: extension must be a SEQUENCE');
                }
