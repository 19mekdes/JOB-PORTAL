 ...
 */
export class CertificateList extends PkiBase {
    ...
    static async createEmpty(options) {
        ...
        const signatureAlgorithmParams = options?.signatureAlgorithmParams ?? {
            type: 'RSASSA_PKCS1_v1_5',
            params: {
                hash: 'SHA-256',
            },
        };
        const signatureAlgorithm = AlgorithmIdentifier.signatureAlgorithm(signatureAlgorithmParams);
        // No revoked certificates for now
        const revokedCertificates = [];
        // Set thisUpdate to now, nextUpdate to +30 days
        const thisUpdate = new Date();
        const nextUpdate = new Date(thisUpdate.getTime() + 30 * 24 * 60 * 60 * 1000);
        ...
        return new CertificateList({
            tbsCertList,
            signatureAlgorithm,
            signatureValue: await signatureAlgorithm.sign(tbsCertList, options.privateKey),
        });
    }
    /**
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE but got ' +
        ...
        if (!(values[0] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for tbsCertList');
        }
        const tbsCertList = TBSCertList.fromAsn1(values[0]);
        // Extract signatureAlgorithm
        if (!(values[1] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for signatureAlgorithm');
        }
        const signatureAlgorithm = AlgorithmIdentifier.fromAsn1(values[1]);
        // Extract signatureValue
        if (!(values[2] instanceof asn1js.BitString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected BIT STRING for signatureValue');
        }
    ...
    static async fetch(url) {
        ...
        if (!res.ok) {
            throw new Error(`Failed to fetch CRL from ${url}: ${res.statusText}`);
        }
