 ...
 * ```
 */
export class ESSCertIDv2 extends PkiBase {
    hashAlgorithm;
    certHash;
    issuerSerial;
    constructor(options) {
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 1 || values.length > 3) {
            throw new Asn1ParseError('Invalid ESSCertIDv2: expected 1 to 3 elements');
        }
        let hashAlgorithm;
        let certHash;
        let issuerSerial;
        ...
        if (!(values[index] instanceof asn1js.OctetString)) {
            throw new Asn1ParseError('Invalid ESSCertIDv2: expected OCTET STRING for certHash');
        }
 ...
 * }
 */
export class SigningCertificateV2 extends PkiBase {
    certs;
    policies;
    constructor(options) {
        super();
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 1 || values.length > 2) {
            throw new Asn1ParseError('Invalid SigningCertificateV2: expected 1 to 2 elements');
        }
        const certsAsn1 = values[0];
        if (!(certsAsn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid SigningCertificateV2: expected SEQUENCE for certs');
        }
        const certs = certsAsn1.valueBlock.value.map((c) => ESSCertIDv2.fromAsn1(c));
        let policies;
        if (values.length > 1) {
            const policiesAsn1 = values[1];
            if (!(policiesAsn1 instanceof asn1js.Sequence)) {
                throw new Asn1ParseError('Invalid SigningCertificateV2: expected SEQUENCE for policies');
            }
    ...
    static async fromCertificates(options) {
        ...
        const essCerts = await Promise.all(options.certificates.map(async (cert) => new ESSCertIDv2({
            hashAlgorithm,
