 ...
 * ```
 */
export class ESSCertID extends PkiBase {
    certHash;
    issuerSerial;
    constructor(options) {
        super();
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 1 || values.length > 2) {
            throw new Asn1ParseError('Invalid ESSCertID: expected 1 to 2 elements');
        }
        let certHash;
        let issuerSerial;
        let index = 0;
        ...
        if (!(values[index] instanceof asn1js.OctetString)) {
            throw new Asn1ParseError('Invalid ESSCertID: expected OCTET STRING for certHash');
        }
 ...
 * }
 */
export class SigningCertificate extends PkiBase {
    certs;
    policies;
    constructor(options) {
        super();
    ...
    toAsn1() {
        ...
        if (this.policies) {
            values.push(new asn1js.Sequence({
    ...
    static fromAsn1(asn1) {
        ...
        const values = asn1.valueBlock.value;
        if (values.length < 1 || values.length > 2) {
            throw new Asn1ParseError('Invalid SigningCertificate: expected 1 to 2 elements');
        }
        const certsAsn1 = values[0];
        if (!(certsAsn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid SigningCertificate: expected SEQUENCE for certs');
        }
        const certs = certsAsn1.valueBlock.value.map((c) => ESSCertID.fromAsn1(c));
        let policies;
        if (values.length > 1) {
            const policiesAsn1 = values[1];
            if (!(policiesAsn1 instanceof asn1js.Sequence)) {
                throw new Asn1ParseError('Invalid SigningCertificate: expected SEQUENCE for policies');
            }
            policies = policiesAsn1.valueBlock.value.map((p) => PolicyInformation.fromAsn1(p));
        }
        return new SigningCertificate({ certs, policies });
    }
    static async fromCertificates(options) {
        const essCerts = await Promise.all(options.certificates.map(async (cert) => new ESSCertID({
            certHash: await cert.getHash('SHA-1'), // RFC 5035 specifies SHA-1
            issuerSerial: new IssuerSerial({
                issuer: new GeneralNames(GeneralName.directoryName.fromName(cert.tbsCertificate.issuer)),
