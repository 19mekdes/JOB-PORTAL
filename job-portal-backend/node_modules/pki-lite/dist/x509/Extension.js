...
import { asn1js, derToAsn1, PkiBase, PkiSequence, PkiSet, } from '../core/PkiBase.js';
import { AccessDescription, AuthorityInfoAccess, } from './extensions/AuthorityInfoAccess.js';
import { BasicConstraints } from './extensions/BasicConstraints.js';
import { ExtKeyUsage } from './extensions/ExtKeyUsage.js';
 ...
 * ```
 */
export class Extension extends PkiBase {
    extnID;
    critical;
    extnValue;
    constructor(options) {
    ...
    static fromAsn1(asn1) {
        ...
        if (asn1.valueBlock.value.length < 2 ||
            ...
            throw new Asn1ParseError('Expected 2 or 3 elements in Extension sequence');
        }
    ...
    static authorityKeyIdentifier(keyIdentifier) {
        return new Extension({
    ...
    static subjectKeyIdentifier(keyIdentifier) {
        return new Extension({
    ...
    static authorityInfoAccess(accessDescriptions) {
        return new Extension({
            ...
            critical: false,
            extnValue: accessDescriptions instanceof AuthorityInfoAccess
                ? accessDescriptions
                : new AuthorityInfoAccess(...accessDescriptions.map((ad) => new AccessDescription(ad))),
        });
    }
    static extendedKeyUsage(options) {
        return new Extension({
            ...
            critical: false,
            extnValue: options instanceof ExtKeyUsage
                ? options
