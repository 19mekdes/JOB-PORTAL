 ...
 * ```
 */
export class ExtendedCertificate extends PkiBase {
    ...
    constructor(options) {
        super();
        const { extendedCertificateInfo, signatureAlgorithm, signature } = options;
    ...
    static fromAsn1(asn1) {
        ...
        if (!(values[0] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for extendedCertificateInfo');
        }
        const extendedCertificateInfo = ExtendedCertificateInfo.fromAsn1(values[0]);
        ...
        if (!(values[2] instanceof asn1js.BitString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected BIT STRING for signature');
        }
