 ...
 */
export class Holder extends PkiBase {
 ...
 * This implementation provides a simplified representation.
 */
export class AttCertIssuer extends PkiBase {
 ...
 * ```
 */
export class AttributeCertificateInfo extends PkiBase {
    version;
    holder;
    issuer;
    signature;
    ...
    toAsn1() {
        ...
        if (this.extensions && this.extensions.length > 0) {
            value.push(new asn1js.Sequence({
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 7) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected at least 7 elements');
        }
        // Extract version
        ...
        if (!(values[1] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for holder');
        }
        const holder = Holder.fromAsn1(values[1]);
        // Extract issuer
        if (!(values[2] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for issuer');
        }
        const issuer = AttCertIssuer.fromAsn1(values[2]);
        // Extract signature algorithm
        if (!(values[3] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for signature');
        }
        const signature = AlgorithmIdentifier.fromAsn1(values[3]);
        // Extract serial number
        if (!(values[4] instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected INTEGER for serialNumber');
        }
        const serialNumber = new Uint8Array(values[4].valueBlock.valueHexView);
        // Extract validity period
        if (!(values[5] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for validityPeriod');
        }
        const validityValues = values[5].valueBlock
            .value;
        if (validityValues.length !== 2) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected 2 elements in validityPeriod');
        }
        if (!(validityValues[0] instanceof asn1js.GeneralizedTime) ||
            !(validityValues[1] instanceof asn1js.GeneralizedTime)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected GeneralizedTime for validity dates');
        }
        ...
        if (!(values[6] instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected SEQUENCE for attributes');
        }
        const attributesValues = values[6].valueBlock
            .value;
        const attributes = attributesValues.map((attr) => Attribute.fromAsn1(attr));
        // Extract optional issuerUniqueID if present
        let issuerUniqueID;
        let extensions;
        if (values.length > 7) {
            if (values[7] instanceof asn1js.BitString) {
                issuerUniqueID = new Uint8Array(values[7].valueBlock.valueHexView);
            }
            // Extract optional extensions if present
            if (values.length > 8 && values[8] instanceof asn1js.Sequence) {
                const extensionsValues = values[8].valueBlock.value;
                extensions = extensionsValues.map((ext) => Extension.fromAsn1(ext));
            }
        }
