 ...
 * ```
 */
export class BasicConstraints extends PkiBase {
    cA;
    pathLenConstraint;
    constructor(options) {
        super();
    ...
    static fromAsn1(asn1) {
        ...
        if (!(cA instanceof asn1js.Boolean)) {
            throw new Asn1ParseError('Invalid BasicConstraints ASN.1 structure: cA is not a Boolean');
        }
        let pathLenConstraint;
        if (sequence.length > 1) {
            const plc = sequence[1];
            if (!(plc instanceof asn1js.Integer)) {
                throw new Asn1ParseError('Invalid BasicConstraints ASN.1 structure: pathLenConstraint is not an Integer');
            }
