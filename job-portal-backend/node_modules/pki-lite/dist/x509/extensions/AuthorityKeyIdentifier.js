 ...
 * }
 */
export class AuthorityKeyIdentifier extends PkiBase {
    keyIdentifier;
    authorityCertIssuer;
    ...
    static fromAsn1(asn1) {
        ...
        if (sequence.valueBlock.value.length === 0) {
            throw new Asn1ParseError('AuthorityKeyIdentifier sequence has no elements');
        }
        let keyIdentifier;
            ...
            switch (tagNumber) {
                case 0: // keyIdentifier [0] IMPLICIT OCTET STRING
                    if (element instanceof asn1js.Constructed) {
                        keyIdentifier = KeyIdentifier.fromAsn1(element.valueBlock.value[0]);
                    }
                    else {
                ...
                case 1: // authorityCertIssuer
                    if (element instanceof asn1js.Constructed) {
                        authorityCertIssuer = GeneralNames.fromAsn1(element.valueBlock.value[0]);
                    }
                    else {
                ...
                case 2: // authorityCertSerialNumber
                    if (element instanceof asn1js.Constructed) {
                        authorityCertSerialNumber = Integer.fromAsn1(element.valueBlock.value[0]);
                    }
                    else {
                    ...
                    break;
                default:
                    throw new Asn1ParseError(`Unexpected tag number ${element.idBlock.tagNumber} in AuthorityKeyIdentifier sequence`);
            }
        }
