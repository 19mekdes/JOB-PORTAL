import { asn1js, PkiBase, derToAsn1, pemToDer, } from '../core/PkiBase.js';
 ...
 * ```
 */
export class ECPrivateKey extends PkiBase {
    /**
    ...
    toAsn1() {
        ...
        if (this.namedCurve) {
            ...
            result.push(new asn1js.Constructed({
                idBlock: {
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 2 || values.length > 4) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected 2-4 elements, got ${values.length}`);
        }
        // Version (must be 1)
        if (!(values[0] instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: version must be INTEGER');
        }
        const version = values[0].valueBlock.valueDec;
        if (version !== 1) {
            throw new Asn1ParseError(`Invalid EC private key version: expected 1, got ${version}`);
        }
        // Private key
        if (!(values[1] instanceof asn1js.OctetString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: privateKey must be OCTET STRING');
        }
        ...
        for (let i = 2; i < values.length; i++) {
            ...
            if (!(value instanceof asn1js.Constructed)) {
                throw new Asn1ParseError('Invalid ASN.1 structure: optional elements must be tagged');
            }
            ...
            if (tagNumber === 0) {
                ...
                if (value.valueBlock.value.length !== 1) {
                    throw new Asn1ParseError('Invalid ASN.1 structure: parameters must contain exactly one element');
                }
                const curveParam = value.valueBlock.value[0];
                if (!(curveParam instanceof asn1js.ObjectIdentifier)) {
                    throw new Asn1ParseError('Invalid ASN.1 structure: namedCurve must be an OBJECT IDENTIFIER');
                }
                ...
                if (value.valueBlock.value.length !== 1) {
                    throw new Asn1ParseError('Invalid ASN.1 structure: publicKey must contain exactly one element');
                }
                const pubKeyParam = value.valueBlock.value[0];
                if (!(pubKeyParam instanceof asn1js.BitString)) {
                    throw new Asn1ParseError('Invalid ASN.1 structure: publicKey must be a BIT STRING');
                }
                publicKey = new Uint8Array(pubKeyParam.valueBlock.valueHexView);
            }
            else {
                throw new Asn1ParseError(`Invalid ASN.1 structure: unexpected tag number ${tagNumber}`);
            }
        }
     ...
     * @returns An ECPrivateKey object
     */
    static forCurve(curve, privateKey, publicKey) {
