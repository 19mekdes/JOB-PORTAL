 ...
 * ```
 */
export class RSAPrivateKey extends PkiBase {
    ...
    constructor(options) {
        super();
        const { modulus, publicExponent, privateExponent, prime1, prime2, exponent1, exponent2, coefficient, version = 0, } = options;
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 9) {
            throw new Asn1ParseError(`Invalid ASN.1 structure: expected at least 9 elements, got ${values.length}`);
        }
        // Version
        ...
        for (let i = 1; i < 9; i++) {
            if (!(values[i] instanceof asn1js.Integer)) {
                throw new Asn1ParseError(`Invalid ASN.1 structure: element at index ${i} must be INTEGER`);
            }
            // Access the value through valueBlock
            const integerBlock = values[i].valueBlock; // Using any to bypass TypeScript checks
            components.push(new Uint8Array(integerBlock.valueHexView ||
