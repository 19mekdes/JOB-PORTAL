 ...
 * }
 */
export class ECDSASignature extends PkiBase {
    r;
    s;
    constructor(options) {
        super();
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length !== 2) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected 2 elements');
        }
        const rAsn1 = values[0];
        const sAsn1 = values[1];
        if (!(rAsn1 instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: r must be INTEGER');
        }
        if (!(sAsn1 instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: s must be INTEGER');
        }
    ...
    static fromRaw(raw) {
        if (raw.length % 2 !== 0) {
            throw new Asn1ParseError('Invalid raw ECDSA signature: length must be even');
        }
