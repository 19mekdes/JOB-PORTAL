 ...
 * ```
 */
export class PrivateKeyInfo extends PkiBase {
    /**
    ...
    static fromAsn1(asn1) {
        ...
        if (asn1.valueBlock.value.length < 3) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected at least 3 elements');
        }
        ...
        if (!(versionAsn1 instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: version must be an INTEGER');
        }
        if (!(algorithmAsn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: algorithm must be a SEQUENCE');
        }
        if (!(privateKeyAsn1 instanceof asn1js.OctetString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: privateKey must be an OCTET STRING');
        }
        const version = versionAsn1.valueBlock.valueDec;
        const algorithm = AlgorithmIdentifier.fromAsn1(algorithmAsn1);
        const privateKey = new Uint8Array(privateKeyAsn1.valueBlock.valueHexView);
        // Check for optional attributes [0] IMPLICIT
        let attributes;
        if (asn1.valueBlock.value.length > 3) {
            const attributesAsn1 = asn1.valueBlock.value[3];
            if (attributesAsn1 instanceof asn1js.Constructed &&
                attributesAsn1.idBlock.tagClass === 3 &&
                attributesAsn1.idBlock.tagNumber === 0) {
                attributes = attributesAsn1.valueBlock.value.map((attrAsn1) => Attribute.fromAsn1(attrAsn1));
            }
        }
