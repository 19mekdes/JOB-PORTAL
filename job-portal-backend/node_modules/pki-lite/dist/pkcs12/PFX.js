 ...
 * ```
 */
export class PFX extends PkiBase {
    /**
    ...
    async getBags(password) {
        if (this.authSafe.contentType.isNot(OIDs.PKCS7.DATA)) {
            throw new Error(`Unsupported authSafe contentType: ${this.authSafe.contentType}`);
        }
    ...
    async getPrivateKeys(password) {
        const bags = await this.getBagsByName('PKCS8_SHROUDED_KEY_BAG', password);
        return await Promise.all(bags.map((bag) => bag.getAs(EncryptedPrivateKeyInfo).decrypt(password)));
    }
    async getX509Certificates(password) {
