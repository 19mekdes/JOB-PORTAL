 ...
 * ```
 */
export class CertBag extends PkiBase {
    /**
    ...
    static fromAsn1(asn1) {
        ...
        if (!(id instanceof asn1js.ObjectIdentifier)) {
            throw new Asn1ParseError('CertBag: certId must be OBJECT IDENTIFIER');
        }
        if (!(val instanceof asn1js.Constructed) ||
            val.idBlock.tagClass !== 3 ||
            val.idBlock.tagNumber !== 0 ||
            val.valueBlock.value.length !== 1) {
            throw new Asn1ParseError('CertBag: certValue must be [0] EXPLICIT with single element');
        }
        return new CertBag({
