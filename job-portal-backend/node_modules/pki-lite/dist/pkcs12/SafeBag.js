 ...
 * ```
 */
export class SafeBag extends PkiBase {
    /**
    ...
    static fromAsn1(asn1) {
        if (!(asn1 instanceof asn1js.Sequence)) {
            throw new Asn1ParseError('SafeBag: expected SEQUENCE but got ' + asn1.constructor.name);
        }
        ...
        if (!(bagVal instanceof asn1js.Constructed) ||
            bagVal.idBlock.tagClass !== 3 ||
            bagVal.idBlock.tagNumber !== 0 ||
            bagVal.valueBlock.value.length !== 1) {
            throw new Asn1ParseError('SafeBag: bagValue must be [0] EXPLICIT with single element');
        }
        let attrs;
        if (values.length === 3) {
            const set = values[2];
