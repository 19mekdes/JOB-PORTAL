...
import { asn1js, derToAsn1, PkiSequence, } from '../core/PkiBase.js';
    ...
    async getBags(password) {
        ...
        for (const contentInfo of this) {
            if (contentInfo.contentType.is(OIDs.PKCS7.ENCRYPTED_DATA)) {
                ...
                if (encryptedData.encryptedContentInfo.contentType.isNot(OIDs.PKCS7.DATA)) {
                    throw new Error(`Unsupported encrypted contentType: ${encryptedData.encryptedContentInfo.contentType}`);
                }
                const safeBags = await encryptedData.encryptedContentInfo.decryptAs(password, SafeContents);
                ...
                throw new Error(`Unsupported contentType in AuthenticatedSafe: ${contentInfo.contentType.friendlyName}`);
            }
        }
        return bags;
    }
    async getBagsByName(bagName, password) {
