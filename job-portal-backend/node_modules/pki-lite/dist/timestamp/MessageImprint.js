 ...
 */
export class MessageImprint extends PkiBase {
    ...
    static fromAsn1(asn1) {
        ...
        if (asn1.valueBlock.value.length !== 2) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected exactly 2 elements');
        }
        const hashAlgorithm = AlgorithmIdentifier.fromAsn1(asn1.valueBlock.value[0]);
        if (!(asn1.valueBlock.value[1] instanceof asn1js.OctetString)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected OCTET STRING for hashedMessage');
        }
        const hashedMessage = new Uint8Array(asn1.valueBlock.value[1].valueBlock.valueHexView);
