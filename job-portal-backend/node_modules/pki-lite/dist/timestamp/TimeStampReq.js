 ...
 */
export class TimeStampReq extends PkiBase {
    ...
    nonce; // Store as bytes to preserve exact value
    ...
    constructor(options) {
        super();
        const { version, messageImprint, reqPolicy, nonce, certReq, extensions, } = options;
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length < 2) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected at least 2 elements');
        }
        // Version
        ...
        if (version !== 1) {
            throw new Asn1ParseError(`Unsupported TimeStampReq version: ${version}`);
        }
        // MessageImprint
    ...
    async request(options) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`TSA request failed (${response.status}): ${errorText}`);
        }
     ...
     * ```
     */
    static fromMessageImprint(messageImprint) {
