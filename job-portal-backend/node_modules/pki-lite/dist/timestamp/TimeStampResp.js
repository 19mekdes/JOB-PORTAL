 ...
 * ```
 */
export class PKIFreeText extends PkiBase {
    ...
    toAsn1() {
        return new asn1js.Sequence({
            value: this.texts.map((text) => new asn1js.Utf8String({ value: text })),
        });
    }
    /**
    ...
    static fromAsn1(asn1) {
        ...
        const texts = asn1.valueBlock.value.map((value) => {
            if (!(value instanceof asn1js.Utf8String)) {
                throw new Asn1ParseError('Invalid ASN.1 structure: expected UTF8String in PKIFreeText');
            }
 ...
 * ```
 */
export class PKIStatusInfo extends PkiBase {
    ...
    static fromAsn1(asn1) {
        ...
        if (values.length === 0) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected at least 1 element');
        }
        // Status
        if (!(values[0] instanceof asn1js.Integer)) {
            throw new Asn1ParseError('Invalid ASN.1 structure: expected INTEGER for status');
        }
        const status = values[0].valueBlock.valueDec;
        ...
        for (let i = 1; i < values.length; i++) {
            ...
            if (value instanceof asn1js.Sequence) {
                ...
                const failInfoBytes = new Uint8Array(value.valueBlock.valueHexView);
                if (failInfoBytes.length > 0) {
                    failInfo = failInfoBytes[0];
                }
            }
        }
     ...
     */
    isSuccess() {
        return (this.status === PKIStatus.GRANTED ||
 ...
 * ```
 */
export class TimeStampResp extends PkiBase {
    status;
    timeStampToken;
    constructor(options) {
        super();
     ...
     */
    toAsn1() {
        const values = [this.status.toAsn1()];
