 ...
 * }
 */
export class BasicOCSPResponse extends PkiBase {
    tbsResponseData;
    signatureAlgorithm;
    signature;
    certs;
    ...
    static fromAsn1(asn1) {
        ...
        if (sequence.valueBlock.value.length < 3 ||
            ...
            throw new Asn1ParseError('Invalid BasicOCSPResponse: expected 3 or 4 elements');
        }
        const tbsResponseData = ResponseData.fromAsn1(sequence.valueBlock.value[0]);
        const signatureAlgorithm = AlgorithmIdentifier.fromAsn1(sequence.valueBlock.value[1]);
        const signatureBlock = sequence.valueBlock.value[2];
        const signature = new Uint8Array(signatureBlock.valueBlock.valueHexView);
        let certs = undefined;
        if (sequence.valueBlock.value.length > 3) {
            const certsContainer = sequence.valueBlock
                .value[3];
            if (certsContainer.idBlock.tagClass !== 3 ||
                certsContainer.idBlock.tagNumber !== 0) {
                throw new Asn1ParseError('Invalid BasicOCSPResponse: expected [0] tag for certs');
            }
            const certsSequence = certsContainer.valueBlock
                .value[0];
            certs = certsSequence.valueBlock.value.map((cert) => Certificate.fromAsn1(cert));
        }
