 ...
 * }
 */
export class ResponseData extends PkiBase {
    version;
    responderID;
    producedAt;
    responses;
    ...
    toAsn1() {
        ...
        values.push(responderIdAsn1, new asn1js.GeneralizedTime({ valueDate: this.producedAt }), new asn1js.Sequence({
    ...
    static fromAsn1(asn1) {
        ...
        if (currentIndex < sequence.valueBlock.value.length &&
            ...
            sequence.valueBlock.value[currentIndex]
                .idBlock.tagNumber === 0) {
            const versionContainer = sequence.valueBlock.value[currentIndex];
        ...
        if (currentIndex >= sequence.valueBlock.value.length) {
            throw new Asn1ParseError('Invalid ResponseData: missing responderID');
        }
        const responderID = ResponderID.fromAsn1(sequence.valueBlock.value[currentIndex]);
        currentIndex++;
        if (currentIndex >= sequence.valueBlock.value.length) {
            throw new Asn1ParseError('Invalid ResponseData: missing producedAt');
        }
        const producedAtBlock = sequence.valueBlock.value[currentIndex];
        const producedAt = producedAtBlock.toDate();
        currentIndex++;
        if (currentIndex >= sequence.valueBlock.value.length) {
            throw new Asn1ParseError('Invalid ResponseData: missing responses');
        }
        const responsesSequence = sequence.valueBlock.value[currentIndex];
        const responses = responsesSequence.valueBlock.value.map((resp) => SingleResponse.fromAsn1(resp));
        currentIndex++;
        let responseExtensions = undefined;
        ...
        if (currentIndex < sequence.valueBlock.value.length &&
            ...
            sequence.valueBlock.value[currentIndex]
                .idBlock.tagNumber === 1) {
            const extensionsContainer = sequence.valueBlock.value[currentIndex];
            const extensionsSequence = extensionsContainer.valueBlock
                .value[0];
            responseExtensions = extensionsSequence.valueBlock.value.map((ext) => Extension.fromAsn1(ext));
        }
