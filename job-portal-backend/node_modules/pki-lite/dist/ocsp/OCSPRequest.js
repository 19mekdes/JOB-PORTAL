 ...
 * ```
 */
export class OCSPRequest extends PkiBase {
    /**
    ...
    toAsn1() {
        const values = [this.tbsRequest.toAsn1()];
    ...
    static fromAsn1(asn1) {
        ...
        const sequence = asn1;
        if (sequence.valueBlock.value.length < 1 ||
            sequence.valueBlock.value.length > 2) {
            throw new Asn1ParseError('Invalid OCSPRequest: expected 1 or 2 elements');
        }
        const tbsRequest = TBSRequest.fromAsn1(sequence.valueBlock.value[0]);
        let optionalSignature = undefined;
        if (sequence.valueBlock.value.length === 2) {
            const signatureContainer = sequence.valueBlock
                .value[1];
            if (signatureContainer.idBlock.tagClass !== 3 ||
                signatureContainer.idBlock.tagNumber !== 0) {
                throw new Asn1ParseError('Invalid OCSPRequest: expected [0] tag for optionalSignature');
            }
            optionalSignature = OCSPSignature.fromAsn1(signatureContainer.valueBlock.value[0]);
        }
    ...
    async request(options) {
        ...
        if (!res.ok) {
            throw new Error(`OCSP request failed with status ${res.status} [${res.statusText}]: ${await res.text()}`);
        }
        if (res.status >= 300) {
            throw new Error(`OCSP request failed with status ${res.status} [${res.statusText}]: ${await res.text()}`);
        }
