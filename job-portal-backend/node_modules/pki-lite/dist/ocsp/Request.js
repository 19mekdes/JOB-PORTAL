 ...
 * }
 */
export class Request extends PkiBase {
    reqCert;
    ...
    toAsn1() {
        const values = [this.reqCert.toAsn1()];
    ...
    static fromAsn1(asn1) {
        ...
        if (sequence.valueBlock.value.length < 1 ||
            ...
            throw new Asn1ParseError('Invalid Request: expected 1 or 2 elements');
        }
        const reqCert = CertID.fromAsn1(sequence.valueBlock.value[0]);
        let singleRequestExtensions = undefined;
        if (sequence.valueBlock.value.length === 2) {
            const extensionsContainer = sequence.valueBlock
                .value[1];
            if (extensionsContainer.idBlock.tagClass !== 3 ||
                extensionsContainer.idBlock.tagNumber !== 0) {
                throw new Asn1ParseError('Invalid Request: expected [0] tag for singleRequestExtensions');
            }
            const extensionsSequence = extensionsContainer.valueBlock
                .value[0];
            singleRequestExtensions = extensionsSequence.valueBlock.value.map((ext) => Extension.fromAsn1(ext));
        }
