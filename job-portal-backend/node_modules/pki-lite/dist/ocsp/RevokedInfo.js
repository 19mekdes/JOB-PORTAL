 ...
 * ```
 */
export class RevokedInfo extends PkiBase {
    revocationTime;
    revocationReason;
    constructor(options) {
        super();
    ...
    static fromAsn1(asn1) {
        ...
        if (sequence.valueBlock.value.length < 1 ||
            ...
            throw new Asn1ParseError('Invalid RevokedInfo: expected 1 or 2 elements');
        }
        ...
        let revocationReason = undefined;
        if (sequence.valueBlock.value.length === 2) {
            const revocationReasonBlock = sequence.valueBlock
                .value[1];
            if (revocationReasonBlock.idBlock.tagClass !== 3 ||
                revocationReasonBlock.idBlock.tagNumber !== 0) {
                throw new Asn1ParseError('Invalid RevokedInfo: expected [0] tag for revocationReason');
            }
            const reasonValue = revocationReasonBlock.valueBlock
                .value[0];
            if (!(reasonValue instanceof asn1js.Enumerated)) {
                throw new Asn1ParseError('Invalid ASN.1 structure: expected ENUMERATED for revocation reason');
            }
