 ...
 * ```
 */
export async function sha384(input) {
 ...
 * ```
 */
export async function aes128cbcEncrypt(key, data, iv = new Uint8Array(16)) {
    if (key.length !== 16) {
 ...
 * ```
 */
export async function aes128cbcDecrypt(key, encrypted, iv = new Uint8Array(16)) {
    if (key.length !== 16) {
        throw new Error(`AES-128 key must be exactly 16 bytes, got ${key.length}`);
    }
 ...
 * ```
 */
export async function aes256cbcEncrypt(fileKey, block, iv = new Uint8Array(16)) {
    return AlgorithmIdentifier.contentEncryptionAlgorithm({
        type: 'AES_256_CBC',
        params: {
            nonce: iv,
 ...
 * ```
 */
export async function aes256cbcDecrypt(fileKey, ciphertext, iv = new Uint8Array(16)) {
    return AlgorithmIdentifier.contentEncryptionAlgorithm({
        type: 'AES_256_CBC',
        params: {
            nonce: iv,
 ...
 * ```
 */
export async function aes256ecbEncrypt(fileKey, data) {
    if (fileKey.length !== 32) {
        throw new Error(`AES-256 key must be exactly 32 bytes, got ${fileKey.length}`);
    }
    return AlgorithmIdentifier.contentEncryptionAlgorithm({
        type: 'AES_256_ECB',
        params: {},
 ...
 * ```
 */
export async function aes256ecbDecrypt(fileKey, encrypted) {
    if (fileKey.length !== 32) {
        throw new Error(`AES-256 key must be exactly 32 bytes, got ${fileKey.length}`);
    }
    return AlgorithmIdentifier.contentEncryptionAlgorithm({
 ...
 * ```
 */
export async function aes128CbcNoPaddingEncrypt(key, data, iv) {
    return AlgorithmIdentifier.contentEncryptionAlgorithm({
        type: 'AES_128_CBC',
        params: {
            nonce: iv,
 ...
 * ```
 */
export async function aes256CbcNoPaddingEncrypt(key, data, iv) {
    return AlgorithmIdentifier.contentEncryptionAlgorithm({
        type: 'AES_256_CBC',
        params: {
            nonce: iv,
 ...
 * ```
 */
export async function aes256CbcNoPaddingDecrypt(key, ciphertext, iv = new Uint8Array(16)) {
    return AlgorithmIdentifier.contentEncryptionAlgorithm({
        type: 'AES_256_CBC',
        params: {
            nonce: iv,
 ...
 * ```
 */
export function deflateData(data) {
...
export function inflateData(data) {
    ...
    if (inflate.err || !inflate.result) {
        ...
        throw new Error(`Inflate error: ${inflate.err} - ${inflate.msg}. Data hex: ${bytesToHex(data)}`); // Log the hex representation of the data for debugging
    }
