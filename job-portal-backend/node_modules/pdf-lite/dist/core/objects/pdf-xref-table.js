...
export class PdfXRefTable extends PdfObject {
    ...
    getEntry(objectNumber) {
        return this.entries.find((entry) => entry.objectNumber.value === objectNumber);
    }
    get lastSection() {
    ...
    tokenize() {
        const innerTokens = this.sortEntriesIntoSections().flatMap(({ section, entries }) => [
            ...section.toTokens(),
        ...
        for (let i = 0; i < innerTokens.length; i++) {
            ...
            const previousIsWhitespace = previousToken instanceof PdfWhitespaceToken;
            const currentIsWhitespace = currentToken instanceof PdfWhitespaceToken;
    ...
    sortEntriesIntoSections() {
        const sections = [];
        let currentSection = null;
        let currentEntries = [];
        const sortedEntries = this.entries.sort((a, b) => a.objectNumber.value - b.objectNumber.value);
        for (let i = 0; i < sortedEntries.length; i++) {
            ...
            if (!last ||
                ...
                currentSection =
                    this.sections.find((x) => x.startObjectNumber.value ===
