...
export class PdfDate extends PdfString {
    ...
    get date() {
        ...
        let date = new Date(Date.UTC(parseInt(year), parseInt(month) - 1, parseInt(day), parseInt(hours), parseInt(minutes), parseInt(seconds)));
        if (tzSign && tzSign !== 'Z') {
            const offsetHours = parseInt(tzHours);
            const offsetMinutes = parseInt(tzMinutes);
            const totalOffsetMinutes = offsetHours * 60 + (isNaN(offsetMinutes) ? 0 : offsetMinutes);
