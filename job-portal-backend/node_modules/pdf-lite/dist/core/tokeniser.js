...
export class PdfByteStreamTokeniser extends IncrementalParser {
    ...
    inXrefTable = false;
    xrefEntryCount = 0;
    lastSectionStartObjectNumber = 0;
    ...
    nextObjectReferenceToken() {
        ...
        return new PdfObjectReferenceToken(objectNumberToken.value, generationNumberToken.value);
    }
    nextStartObjectToken() {
        ...
        return new PdfStartObjectToken(objectNumberToken.value, generationNumberToken.value, offset);
    }
    nextStartArrayToken() {
    ...
    nextXRefTableSectionStartToken() {
        ...
        this.xrefEntryCount = 0;
        return new PdfXRefTableSectionStartToken(firstObjectNumberToken, entryCountToken);
    }
    nextXRefTableEntryToken() {
        ...
        return new PdfXRefTableEntryToken(byteOffsetToken, generationNumberToken, this.lastSectionStartObjectNumber + this.xrefEntryCount++, // Increment and use the current count
        inUse);
    }
    nextToken(root = false) {
        ...
        if (this.inStream) {
            ...
            return this.oneOf(this.nextStartDictionaryToken, this.nextHexadecimalToken);
        }
            ...
            if (this.inXrefTable) {
                return this.oneOf(this.nextXRefTableEntryToken, this.nextXRefTableSectionStartToken);
            }
            return this.oneOf(this.nextStartObjectToken, this.nextObjectReferenceToken, this.nextNumberToken);
        }
            ...
            return this.oneOf(this.nextStartStreamToken, this.nextStartXRefToken);
        }
        else if (byte === ByteMap.x) {
            return this.nextXRefTableStartToken();
        }
        else {
            throw new Error(`[offset: ${this.inputOffset}] Unrecognised token starting with byte: ${byte} (root: ${root}) (char: ${String.fromCharCode(byte ?? 0)})`);
        }
    }
    parse() {
    ...
    static isDigit(byte) {
        return (byte !== null &&
            ((byte >= 0x30 && byte <= 0x39) ||
                byte === ByteMap.MINUS ||
                byte === ByteMap.DOT)); // 0-9 or 0x2d (minus) or 0x2E (.)
    }
    static isNameEnd(byte) {
        if (byte === null) {
            return false;
        }
        return (byte === ByteMap.SLASH ||
 ...
 */
export function* objectToTokens(object) {
 ...
 */
export class PdfObjectTokeniser extends Parser {
    buffer = [];
    /**
