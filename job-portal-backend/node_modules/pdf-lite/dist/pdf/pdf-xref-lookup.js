...
export class PdfXrefLookup {
    ...
    constructor(options) {
        ...
        if (options?.object) {
            if (options.object instanceof PdfXRefTable) {
                ...
                throw new Error('Provided object is not a valid XRef table or stream');
            }
        }
        else {
    ...
    setPrev(xref) {
        ...
        if (xref.offset === this.offset) {
            throw new Error('Cannot set XRef lookup previous to another lookup with the same offset');
        }
        this.prev = xref;
        ...
        for (let i = 0; i < objects.length; i++) {
            ...
            if (obj instanceof PdfXRefTable) {
                ...
                if (!(trailer instanceof PdfTrailer)) {
                    throw new Error('Expected PdfTrailer after PdfXRefTable in objects');
                }
     ...
     */
    static fromXrefTable(xrefTable, trailer, objects) {
        ...
        if (xrefStmOffset instanceof PdfNumber && objects) {
            ...
            const xrefStreamObj = objects.find((obj) => obj instanceof PdfIndirectObject &&
                obj.content instanceof PdfStream &&
                obj.content.isType('XRef') &&
    ...
    update() {
        ...
        if (this.object instanceof PdfXRefTable) {
            ...
            if (compressedEntries.length > 0) {
                ...
                this.trailerDict.set('XRefStm', new PdfNumber(this.hybridXRefStream.offset));
            }
            else {
            ...
            this.object.content = PdfXRefStream.fromEntries(this.entriesValues, this.trailerDict);
            return this.object;
        }
        let newObject;
        if (this.type === 'table') {
            ...
            newObject = new PdfIndirectObject({
                content: PdfXRefStream.fromEntries(this.entriesValues, this.trailerDict),
            });
        }
    ...
    addObject(newObject, options) {
        ...
        if (options?.indexInStream !== undefined &&
            options?.parentObjectNumber !== undefined) {
            if (newObject.generationNumber !== 0) {
                throw new Error('Object streams cannot contain objects with generation number other than 0');
            }
            this.entries.set(newObject.objectNumber, new PdfXRefStreamCompressedEntry({
            ...
            this.entries.set(newObject.objectNumber, new PdfXRefTableEntry({
                objectNumber: newObject.objectNumber,
    ...
    removeObject(object) {
        ...
        for (const entry of Object.keys(trailerValues)) {
