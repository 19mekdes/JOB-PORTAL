 ...
 * ```
 */
export async function decryptUserPasswordRc4_40(ownerPw, ownerKey) {
    const ownerPad = padPassword(ownerPw);
    const digest = await md5(ownerPad);
    const rc4Key = digest.slice(0, 5); // 40-bit key
    return await removePdfPasswordPadding(await rc4EncryptWithKey(rc4Key, ownerKey));
}
/**
 ...
 * ```
 */
export async function computeORc4_40(ownerPw, userPw) {
    const ownerPad = padPassword(ownerPw);
    const digest = await md5(ownerPad);
    const rc4Key = digest.slice(0, 5); // 40-bit key
 ...
 * ```
 */
export async function computeURc4_40(userPw, oValue, permissions, fileId) {
    const encryptionKey = await computeEncryptionKeyRc4_40(userPw, oValue, permissions, fileId);
