...
export class PdfPublicKeySecurityHandler extends PdfSecurityHandler {
    ...
    async initKeys(seed = this.seed) {
        ...
        const key = digest.slice(0, this.standardSecurityHandler.getKeyBits() / 8);
    ...
    async write() {
        ...
        this.dict.set('Recipients', new PdfArray(recipientsData.map((data) => new PdfHexadecimal(data))));
    }
    /**
     ...
     */
    async getRecipientsPkcs7(data) {
        return await Promise.all(this.recipients.map((recipient) => {
     ...
     */
    readEncryptionDictionary(dictionary) {
        this.standardSecurityHandler =
        ...
        this.recipientsCms = Promise.resolve(recipients.items.map((value) => {
    ...
    async getSeed() {
        for (const privateKey of this.recipients
            ...
            for (const cms of await this.recipientsCms) {
                const output = await this.extractSeedAndPermissions(cms, privateKey);
                if (!output)
                    continue;
    ...
    async decrypt(type, data, objectNumber, generationNumber) {
        await this.getSeed();
        return this.standardSecurityHandler.decrypt(type, data, objectNumber, generationNumber);
    }
    /**
    ...
    async encrypt(type, data, objectNumber, generationNumber) {
        await this.initKeys();
        return this.standardSecurityHandler.encrypt(type, data, objectNumber, generationNumber);
    }
    /**
     ...
     */
    async computeObjectKey(objectNumber, generationNumber) {
        await this.getSeed();
        return this.standardSecurityHandler.computeObjectKey(objectNumber, generationNumber);
    }
    /**
     ...
     */
    async extractSeedAndPermissions(contentInfoBytes, privateKey) {
