...
export class PdfV4SecurityHandler extends PdfV2SecurityHandler {
    ...
    constructor(options) {
        super(options);
        this.setCryptFilter('StdCF', new AesV2CryptFilter({
            authEvent: 'DocOpen',
        }));
     ...
     * @param name - The crypt filter name.
     */
    setCryptFilterForType(type, name) {
     ...
     */
    async computeObjectKey(objectNumber, generationNumber, algorithm) {
        assert(objectNumber !== undefined, 'Object number is required to derive the key');
        assert(generationNumber !== undefined, 'Generation number is required to derive the key');
        ...
        const key = await deriveObjectKey(this.masterKey, objectNumber, generationNumber, algorithm === 'AES-128-CBC');
        return key;
    }
    /**
     ...
     */
    async getCipher(objectNumber, generationNumber) {
    ...
    readEncryptionDictionary(encryptionDictionary) {
        ...
        if (encryptionDictionary.get('CF')) {
            ...
            for (const [key, cf] of Object.entries(cfEntries)) {
                assert(cf instanceof PdfDictionary, 'Crypt filter must be a dictionary');
                const cfm = cf.get('CFM');
                const authEvent = cf.get('AuthEvent');
                const length = cf.get('Length');
                assertIfDefined(cfm, cfm instanceof PdfName, 'CFM must be a name');
                assertIfDefined(authEvent, authEvent instanceof PdfName, 'AuthEvent must be a name');
                assertIfDefined(length, length instanceof PdfNumber, 'Length must be a number');
                assert(cfm, 'CFM is required in crypt filter dictionary');
                if (cfm.value === 'None') {
                    this.setCryptFilter(key, new IdentityCryptFilter({
                        authEvent: authEvent?.value,
                    }));
                }
                else if (cfm.value === 'V2') {
                    this.setCryptFilter(key, new V2CryptFilter({
                        authEvent: authEvent?.value,
                        length: length.value,
                    }));
                }
                else if (cfm.value === 'AESV2') {
                    this.setCryptFilter(key, new AesV2CryptFilter({
                        authEvent: authEvent?.value,
                    }));
                }
                else if (cfm.value === 'AESV3') {
                    this.setCryptFilter(key, new AesV3CryptFilter({
                        authEvent: authEvent?.value,
                    }));
                }
                else {
        ...
        if (encryptionDictionary.values['StmF']) {
            ...
            assert(this.cryptFilters.has(stmF.value), `StmF references unknown crypt filter: ${stmF.value}`);
        ...
        if (encryptionDictionary.values['StrF']) {
            ...
            assert(this.cryptFilters.has(strF.value), `StrF references unknown crypt filter: ${strF.value}`);
        ...
        if (encryptionDictionary.values['EFF']) {
            ...
            assert(this.cryptFilters.has(eff.value), `EFF references unknown crypt filter: ${eff.value}`);
     ...
     */
    encrypt(type, data, objectNumber, generationNumber) {
     ...
     */
    decrypt(type, data, objectNumber, generationNumber) {
        const cryptFilter = this.getCryptFilterByType(type);
        if (cryptFilter) {
