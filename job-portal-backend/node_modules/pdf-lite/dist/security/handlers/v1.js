...
export class PdfV1SecurityHandler extends PdfStandardSecurityHandler {
     ...
     */
    async computeOwnerKey() {
        return await computeORc4_40(this.ownerPassword ?? this.password, this.password);
    }
    /**
    ...
    async computeUserKey() {
        ...
        return await computeURc4_40(this.password, this.ownerKey, this.permissions, this.documentId.items[0].bytes);
    }
    /**
    ...
    async computeMasterKey() {
        ...
        assert(this.encryptMetadata !== undefined, 'Encrypt metadata is required');
        if (this.ownerPassword) {
            const recoverPass = await this.recoverUserPassword(this.ownerPassword);
            return await computeMasterKey(stringToBytes(recoverPass), this.ownerKey, this.permissions, this.documentId.items[0].bytes, this.getKeyBits(), this.encryptMetadata, this.getRevision());
        }
        else {
            ...
            assert(this.userKey &&
                ...
                expectedUValue
                    ...
                    .every((byte, index) => byte === this.userKey[index]), 'Incorrect user password');
        }
        return await computeMasterKey(this.password, this.ownerKey, this.permissions, this.documentId.items[0].bytes, this.getKeyBits(), this.encryptMetadata, this.getRevision());
    }
    /**
     ...
     */
    async recoverUserPassword(ownerPassword) {
        ownerPassword ||= this.ownerPassword;
        const password = await decryptUserPasswordRc4_40(stringToBytes(ownerPassword), this.ownerKey);
