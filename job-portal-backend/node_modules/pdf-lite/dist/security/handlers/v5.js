...
export class PdfV5SecurityHandler extends PdfV4SecurityHandler {
    ...
    constructor(options) {
        ...
        this.setCryptFilter('StdCF', new AesV3CryptFilter({
            authEvent: 'DocOpen',
        }));
    ...
    async initKeys() {
        this.fileKey ||= (async () => {
            if (this.userEncryptedFileKey &&
                this.ownerEncryptedFileKey &&
                this.userKey &&
                this.ownerKey &&
                this.perms) {
                const derivedFileKey = await getFileKey(this.password, this.ownerPassword ?? this.password, this.userKey, this.userEncryptedFileKey, this.ownerKey, this.ownerEncryptedFileKey);
            ...
            const { O, OE } = await generateOandOe(this.ownerPassword ?? this.password, U, fileKey);
            this.ownerKey = O;
            this.ownerEncryptedFileKey = OE;
            this.perms = await this.buildPermsEntry(this.permissions ?? 0, this.encryptMetadata, fileKey);
            return fileKey;
        })();
        await this.fileKey;
    }
     ...
     */
    async buildPermsEntry(flags, encryptMetadata, fileKey) {
    ...
    readEncryptionDictionary(encryptionDictionary) {
        ...
        assert(values['UE'] instanceof PdfHexadecimal ||
            values['UE'] instanceof PdfString, 'Invalid or missing /UE entry');
        assert(values['OE'] instanceof PdfHexadecimal ||
            values['OE'] instanceof PdfString, 'Invalid or missing /OE entry');
        assert(values['Perms'] instanceof PdfString ||
            values['Perms'] instanceof PdfHexadecimal, 'Invalid or missing /Perms entry');
    ...
    async recoverUserPassword(ownerPassword) {
        throw new Error('Recovering user password from owner password is not supported for AES-256');
    }
}
