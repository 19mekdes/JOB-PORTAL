 ...
 * ```
 */
export function createFromDictionary(dict, options) {
    const { password, ownerPassword, recipients = [], documentId, } = options ?? {};
    ...
    if (filter.value === 'Standard') {
        ...
        const pubSecHandler = new PdfPublicKeySecurityHandler({
            standardSecurityHandler: createStandardSecurityHandlerFromDictionary(dict, {
                password,
                ownerPassword,
            }),
            recipients,
 ...
 * ```
 */
export function createStandardSecurityHandlerFromDictionary(dict, options) {
    const { password, ownerPassword, documentId = dict.get('ID'), } = options ?? {};
    const V = dict.get('V');
    const R = dict.get('R');
    ...
    assertIfDefined(Length, Length instanceof PdfNumber, 'Length must be a number if defined');
    const version = V.value;
    const revision = R.value;
    let standardSecurityHandler;
    if (version === 1) {
        ...
        throw new Error(`Unsupported standard security handler: V=${version} R=${revision}`);
    }
 ...
 * ```
 */
export function getCryptFilter(method) {
    if (method === 'none') {
 ...
 * ```
 */
export function createFromEncryptionOptions(options) {
    ...
    let uniqueCryptFilters = new Set();
    const cryptFilterForType = {};
    if (options.method &&
    ...
    if (handler instanceof PdfV4SecurityHandler) {
        ...
        for (let i = 0; i < uniqueCryptFiltersArray.length; i++) {
            ...
            const name = cryptFilter instanceof IdentityCryptFilter
                ? 'Identity'
                : `CF${i + 1}`;
            handler.setCryptFilter(name, cryptFilter);
            for (const type in cryptFilterForType) {
                if (cryptFilterForType[type] ===
                    uniqueCryptFiltersArray[i]) {
                    handler.setCryptFilterForType(type, name);
                }
            }
        }
    }
