...
export class PdfFont extends PdfIndirectObject {
     ...
     * @returns A PdfFont instance ready to be written to the PDF
     */
    static fromTrueTypeData(fontData, fontName, descriptor) {
        ...
        fontDescriptorDict.set('FontWeight', new PdfNumber(descriptor.fontWeight));
        fontDescriptorDict.set('Flags', new PdfNumber(descriptor.flags));
        fontDescriptorDict.set('FontBBox', new PdfArray([
        ...
        fontDescriptorDict.set('ItalicAngle', new PdfNumber(descriptor.italicAngle));
     ...
     * @returns A PdfFont instance ready to be written to the PDF
     */
    static fromType0Data(fontData, fontName, descriptor, unicodeMappings) {
        const font = new PdfFont({
            fontName,
            encoding: 'Identity-H',
            descriptor,
     ...
     */
    static generateToUnicodeCMap(mappings) {
        const lines = [];
        ...
        const sortedMappings = Array.from(mappings.entries()).sort((a, b) => a[0] - b[0]);
     ...
     */
    static buildCIDWidthArray(widths) {
        const items = [];
        for (const entry of widths) {
            if ('width' in entry) {
                ...
                items.push(new PdfArray(entry.widths.map((w) => new PdfNumber(w))));
            }
        }
