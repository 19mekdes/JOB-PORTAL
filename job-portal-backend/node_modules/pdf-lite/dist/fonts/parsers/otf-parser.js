...
export class OtfParser {
    ...
    readTag(offset) {
        return String.fromCharCode(this.data.getUint8(offset), this.data.getUint8(offset + 1), this.data.getUint8(offset + 2), this.data.getUint8(offset + 3));
    }
    getTable(tag) {
            ...
            if ((platformId === 3 && encodingId === 1) ||
                ...
                if (platformId === 3) {
                    for (let j = 0; j < length; j += 2) {
                        str += String.fromCharCode(this.data.getUint16(strStart + j));
                    }
                }
                else {
                    for (let j = 0; j < length; j++) {
                        str += String.fromCharCode(this.data.getUint8(strStart + j));
                    }
                }
                names[nameId] = str;
            }
    ...
    parseCmapFormat4(offset) {
        ...
        for (let i = 0; i < segCount; i++) {
            ...
            const idRangeOffsetValue = this.data.getUint16(idRangeOffset + i * 2);
            if (endCode === 0xffff)
                break;
            for (let charCode = startCode; charCode <= endCode; charCode++) {
                ...
                if (idRangeOffsetValue === 0) {
                    ...
                    const glyphIdOffset = idRangeOffset +
                        i * 2 +
                        idRangeOffsetValue +
    ...
    parseHmtx() {
        ...
        if (numHMetrics > 0) {
            const lastWidth = this.data.getUint16(offset + (numHMetrics - 1) * 4);
    ...
    estimateStemV(weightClass) {
        return estimateStemVFromWeight(weightClass);
    }
}
export function estimateStemVFromWeight(weightClass) {
    if (weightClass <= 100)
        return 25;
    if (weightClass <= 200)
        return 35;
