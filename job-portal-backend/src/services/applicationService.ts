...
export class ApplicationService {
  ...
  async applyForJob(
    ...
    if (!job) {
      throw new NotFoundError('Job not found or is no longer accepting applications')
    }
    ...
    if (!pendingStatus) {
      throw new ValidationError('Application status configuration error')
    }
  ...
  async getMyApplications(seekerUserId: string, filters: ApplicationFilters = {}) {
    ...
    const orderBy = sort === 'recent' 
      ? { applied_at: 'desc' as const }
      : { applied_at: 'asc' as const }
  ...
  async updateApplicationStatus(
    ...
    if (finalStatuses.includes(currentStatus!.status_name)) {
      throw new ValidationError(`Cannot update an application that is already ${currentStatus!.status_name.toLowerCase()}`)
    }
  ...
  async addApplicationNote(employerUserId: string, applicationId: string, noteText: string) {
    ...
    if (!application || application.job.employer_id !== employerProfile.id) {
      throw new AppError('Not authorized to add notes to this application', 403)
    }
  ...
  async getApplicationNotes(employerUserId: string, applicationId: string) {
    const employerProfile = await this.prisma.employerProfile.findUnique({
      where: { user_id: employerUserId }
    })
    ...
    if (!application || application.job.employer_id !== employerProfile.id) {
      throw new AppError('Not authorized to view notes for this application', 403)
    }
  ...
  async withdrawApplication(seekerUserId: string, applicationId: string) {
    const seekerProfile = await this.prisma.jobSeekerProfile.findUnique({
      where: { user_id: seekerUserId }
    })
    ...
    if (finalStatuses.includes(currentStatus!.status_name)) {
      throw new ValidationError(`Cannot withdraw an application that is ${currentStatus!.status_name.toLowerCase()}`)
    }
  ...
  private async getApplicationStatsForSeeker(seekerId: string): Promise<ApplicationStats> {
  ...
  private async getApplicationStatsForJob(jobId: string): Promise<ApplicationStats> {
    const stats = await this.prisma.jobApplication.groupBy({
      by: ['status_id'],
      where: { job_id: jobId },
      _count: true
  ...
  async bulkUpdateStatus(
    ...
    if (applications.length !== applicationIds.length) {
      throw new AppError('Some applications are not authorized for bulk update', 403)
    }
  ...
  async exportApplicationsToCSV(employerUserId: string, jobId?: string) {
    const employerProfile = await this.prisma.employerProfile.findUnique({
      where: { user_id: employerUserId }
    })
