...
async function sendApplicationStatusEmail(
  ...
  const html = `
    ...
    <head>
      <style>
        ...
        .status { display: inline-block; padding: 5px 15px; border-radius: 20px; font-weight: bold; background: ${
      ...
      <div class="container">
        ...
        <div class="content">
          ...
          <p style="text-align: center;">
            ...
            }/applications" class="button">View Applications</a>
          </p>
        </div>
        <div class="footer">
  ...
  async function sendNewJobAlertEmail(
    ...
    <div style="border: 1px solid #e5e7eb; padding: 15px; margin-bottom: 15px; border-radius: 8px;">
      ...
      <p style="color: #6b7280; font-size: 14px;">Salary: ${
        job.salary_range || "Competitive"
      }</p>
      <a href="${process.env.FRONTEND_URL}/jobs/${
          job.id
        }" style="color: #2563eb; text-decoration: none;">View Details →</a>
    </div>
  `
      )
      .join("");
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <h3>Hello ${userName},</h3>
          <p>We found ${jobs.length} new job${
      jobs.length > 1 ? "s" : ""
          ...
          <p style="text-align: center; margin-top: 30px;">
            <a href="${
              process.env.FRONTEND_URL
            }/jobs" class="button">Browse All Jobs</a>
          </p>
        </div>
        <div class="footer">
          ...
          <p><a href="${
            process.env.FRONTEND_URL
          }/jobseeker/preferences">Manage Preferences</a></p>
        </div>
      </div>
    </body>
    </html>
  ...
  try {
    ...
    if (user_type === "Job Seeker") {
      ...
      await prisma.employerProfile.create({
        data: {
          ...
          industry_id: defaultIndustry?.id || 1,
        },
      });
    }
    ...
    res
      ...
      .json({
        success: true,
        token,
        user: { id: user.id, email: user.email, user_type, full_name },
      });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
...
app.post("/api/auth/login", async (req: Request, res: Response) => {
  try {
    ...
    const fullName =
      user.seeker_profile?.full_name || user.employer_profile?.company_name;
    res.json({
      success: true,
      token,
      user: {
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        "📤 Returning profile type:",
        user.seeker_profile ? "Job Seeker" : "Employer"
      );
      if (user.seeker_profile) {
...
app.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (portfolio_url !== undefined && portfolio_url !== "")
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!profile || !profile.cover_image)
        return res
          .status(404)
          .json({ success: false, message: "Cover image not found" });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!req.file)
        return res
          .status(400)
          .json({ success: false, message: "No banner file uploaded" });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!employer || !employer.cover_image)
        return res
          .status(404)
          .json({ success: false, message: "Company banner not found" });
      ...
      res.json({
        success: true,
        message: "Company banner removed successfully",
      });
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
      ...
      for (const employer of employers) {
        ...
        companies.push({
          ...
          company_name:
            ...
            "Unnamed Company",
          company_description:
            employer.employer_profile?.company_description || null,
          ...
          is_active:
            employer.is_active !== undefined ? employer.is_active : true,
          created_at: employer.created_at,
          industry_name:
            employer.employer_profile?.industry?.industry_name || null,
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          user_id: company.id,
          company_name:
            company.employer_profile?.company_name || company.full_name,
...
// Verify company
app.put(
  "/api/super-admin/companies/:id/verify",
  authMiddleware,
  superAdminMiddleware,
  async (req: Request, res: Response) => {
    try {
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=backup_${id}.sql`
      );
      res.send(content);
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (full_name) {
        updateData.full_name = full_name;
        console.log(
          `   Updating name: ${existingUser.full_name} -> ${full_name}`
        );
      }
  ...
  try {
    ...
    if (!user.employer_profile.is_verified) {
      return res.status(403).json({
        success: false,
        message:
          "Your company must be verified before posting jobs. Please contact support to complete verification.",
    ...
    try {
      ...
      await prisma.notification.create({
        data: {
          user_id: job.employer.user_id,
          title: "Job Rejected",
          message: `Your job "${job.title}" was rejected.${
    ...
    try {
      ...
      const updatedJob = await prisma.jobPost.update({
        ...
        data: {
          ...
          is_remote:
            is_remote !== undefined ? is_remote : existingJob.is_remote,
...
app.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const isAdmin =
        user?.user_type?.type_name === "Admin" ||
        user?.user_type?.type_name === "Super Admin";
      if (!isOwner && !isAdmin)
        return res
          .status(403)
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const activeJobs = jobs.filter(
        (job) => job.status?.status_name === "Open"
      ).length;
      const closedJobs = jobs.filter(
        (job) => job.status?.status_name === "Closed"
      ).length;
      const draftJobs = jobs.filter(
        (job) => job.status?.status_name === "Draft"
      ).length;
      ...
      if (job.employer?.user?.email) {
        try {
          ...
          <body>
            <div class="container">
              <div class="header">
                <h2>Job ${
                  statusText === "approved" ? "Approved" : "Update"
                }</h2>
              </div>
              <div class="content">
                <h3>Hello ${job.employer.company_name},</h3>
                <p>Your job posting <strong>"${
                  job.title
                }"</strong> has been ${statusText} by an administrator.</p>
                ${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ""}
                <p>You can view your job posting in your employer dashboard.</p>
                <a href="${
                  process.env.FRONTEND_URL || "http://localhost:5173"
                }/employer/jobs" style="background: #2563eb; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; margin-top: 20px;">View Jobs</a>
        ...
        `;
          ...
          await transporter.sendMail({
            ...
            subject: `Job ${
              statusText === "approved" ? "Approved" : "Status Update"
            }: ${job.title}`,
            html: emailHtml,
          });
      ...
      await prisma.notification.create({
        data: {
          ...
          message: `Your job "${job.title}" has been ${
            status === "approved" ? "approved and is now live" : "rejected"
          }.${reason ? ` Reason: ${reason}` : ""}`,
          type: "job_moderation",
          created_at: new Date(),
        },
...
app.post("/api/contact", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `✅ Contact form processed for ${email} -> Sent to mekdesw60@gmail.com`
    );
    ...
    try {
      ...
      const preferences = await prisma.notificationPreference.upsert({
        ...
        update: {
          ...
          application_updates:
            application_alerts !== undefined ? application_alerts : undefined,
          new_job_alerts: job_alerts !== undefined ? job_alerts : undefined,
          marketing_emails:
            marketing_emails !== undefined ? marketing_emails : undefined,
          push_notifications:
            push_notifications !== undefined ? push_notifications : undefined,
          status_changes:
            status_changes !== undefined ? status_changes : undefined,
          weekly_digest:
            weekly_digest !== undefined ? weekly_digest : undefined,
          updated_at: new Date(),
        },
        create: {
          user_id: req.user!.id,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!statusRecord) {
        return res
          .status(404)
          .json({ success: false, message: `Status '${status}' not found` });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!openStatus) {
        return res
          .status(404)
          .json({ success: false, message: "Open status not found" });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!closedStatus) {
        return res
          .status(404)
          .json({ success: false, message: "Closed status not found" });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        updateData.employment_type_id =
          typeof employment_type_id === "string"
            ? parseInt(employment_type_id)
            : employment_type_id;
      }
      ...
      if (
        ...
        updateData.industry_id =
          typeof industry_id === "string" ? parseInt(industry_id) : industry_id;
      }

      // Handle status_id - ensure it's a number
      if (status_id !== undefined && status_id !== null && status_id !== "") {
        updateData.status_id =
          typeof status_id === "string" ? parseInt(status_id) : status_id;
      }
      ...
      if (
        ...
        updateData.salary_min =
          typeof salary_min === "string" ? parseFloat(salary_min) : salary_min;
      }
      if (
        salary_max !== undefined &&
        salary_max !== null &&
        ...
        updateData.salary_max =
          typeof salary_max === "string" ? parseFloat(salary_max) : salary_max;
      }
    ...
    try {
      ...
      if (!statusRecord) {
        return res
          ...
          .json({
            success: false,
            message: `Status '${status}' not found in database`,
          });
      }
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        currentUser?.user_type?.type_name !== "Admin" &&
      ...
      console.log(
        `   Users: ${totalUsers} (Employers: ${totalEmployers}, Job Seekers: ${totalJobSeekers})`
      );
      console.log(
        `   Jobs: ${totalJobs} (Open: ${openJobs}, Closed: ${closedJobs})`
      );
      console.log(
        `   Applications: ${totalApplications} (Pending: ${pendingApplications})`
      );
    ...
    try {
      ...
      for (const user of users) {
        ...
        result.push({
          ...
          location:
            user.seeker_profile?.location ||
            user.employer_profile?.location ||
            null,
          user_type: user.user_type,
          is_active: user.is_active,
          created_at: user.created_at,
      ...
      const stats = {
        ...
        jobSeekers: users.filter((u) => u.user_type?.type_name === "Job Seeker")
          .length,
        employers: users.filter((u) => u.user_type?.type_name === "Employer")
          .length,
        admins: users.filter((u) => u.user_type?.type_name === "Admin").length,
        superAdmins: users.filter(
          (u) => u.user_type?.type_name === "Super Admin"
        ).length,
      };
      ...
      if (existingSettings) {
        ...
        settings = await prisma.systemSetting.create({
          data: {
            ...
            max_applications_per_seeker:
              settingsData.maxApplicationsPerSeeker || 100,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const stats = {
        ...
        pending: await prisma.jobApplication.count({
          where: { status: { status_name: "Pending" } },
        }),
        reviewed: await prisma.jobApplication.count({
          where: { status: { status_name: "Reviewed" } },
        }),
        shortlisted: await prisma.jobApplication.count({
          where: { status: { status_name: "Shortlisted" } },
        }),
        interview: await prisma.jobApplication.count({
          where: { status: { status_name: "Interview" } },
        }),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        <head>
          <style>
            ...
            .header { background: linear-gradient(135deg, #2563eb, #1e40af); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0; }
            .content { background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; }
            .status { display: inline-block; padding: 8px 20px; border-radius: 25px; font-weight: bold; background: ${
              statusColors[status] || "#6b7280"
            }; color: white; }
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>Your application status for <strong>${
          ...
          application.job.employer.company_name
              ...
              <div style="text-align: center;">
                <a href="${
                  process.env.FRONTEND_URL || "http://localhost:5173"
                }/applications/${id}" class="button">View Application Details</a>
              </div>
      ...
      `;
        ...
        console.log(
          `✅ Status update email sent to ${application.seeker.user.email}`
        );
      } catch (emailError) {
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>Your job posting <strong>"${
                job.title
              }"</strong> has been <strong>${status.toLowerCase()}</strong> by an administrator.</p>
              ${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ""}
              <div style="text-align: center;">
                <a href="${
                  process.env.FRONTEND_URL || "http://localhost:5173"
                }/employer/jobs/${jobId}" class="button">View Job Details</a>
              </div>
              <p>Best regards,<br><strong>Job Portal Team</strong></p>
            </div>
            <div class="footer">
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const formattedAdmins = admins.map((admin) => ({
        ...
        email: admin.email,
        phone:
          admin.seeker_profile?.phone || admin.employer_profile?.phone || null,
...
app.put(
  "/api/super-admin/admins/:adminId/suspend",
  authMiddleware,
  superAdminMiddleware,
  async (req: Request, res: Response) => {
    try {
      ...
      if (adminId === req.user!.id) {
        return res
          .status(400)
          .json({ success: false, message: "Cannot suspend your own account" });
      }
...
app.put(
  "/api/super-admin/admins/:adminId/activate",
  authMiddleware,
  superAdminMiddleware,
  async (req: Request, res: Response) => {
    try {
    ...
    try {
      ...
      if (currentRole !== "Admin" && currentRole !== "Super Admin") {
        return res.status(403).json({
          success: false,
          message:
            "Access denied. Only Admin or Super Admin can update user status.",
        });
      }
      ...
      // Admin can ONLY suspend/activate Job Seekers and Employers
        ...
        if (targetRole !== "Job Seeker" && targetRole !== "Employer") {
          return res.status(403).json({
            ...
            } ${targetRole} accounts. Only Job Seekers and Employers can be managed.`,
            allowed_roles: ["Job Seeker", "Employer"],
            your_role: currentRole,
            target_role: targetRole,
          });
      ...
      if (targetRole === "Super Admin" && !is_active) {
        ...
        if (superAdminCount <= 1) {
          return res.status(403).json({
            success: false,
            message:
              "Cannot suspend the last active Super Admin account. This would lock out all administrators.",
          });
        }
      }
      ...
      try {
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0; font-size: 28px;">Account ${
                is_active ? "Activated" : "Suspended"
              }</h1>
            </div>
            <div class="content">
      ...
      `;
        ...
        console.log(
          `✅ ${is_active ? "Activation" : "Suspension"} email sent to ${
            targetUser.email
          }`
        );
      } catch (emailError) {
      ...
      for (const admin of allOtherAdmins) {
        await prisma.notification.create({
          data: {
            ...
            message: `${currentUser.email} (${currentRole}) ${
              is_active ? "activated" : "suspended"
            } user ${targetUser.email} (${targetRole})${
              reason ? `. Reason: ${reason}` : ""
            }`,
            type: "admin_activity",
            metadata: {
      ...
      console.log(
        `📝 Notified ${allOtherAdmins.length} other admins about this action`
      );

      // ========== LOG ACTION FOR AUDIT ==========
      console.log(
        `📝 AUDIT: ${currentRole} (${currentUser.email}) ${
          is_active ? "activated" : "suspended"
        } user ${targetUser.email} (${targetRole})`
      );
      if (reason) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (targetRole === "Super Admin") {
        ...
        if (superAdminCount <= 1) {
          return res.status(403).json({
            success: false,
            message:
              "Cannot delete the last Super Admin account. This would lock out all administrators.",
          });
        }
      }

      console.log(
        `📝 AUDIT: ${currentRole} (${currentUser.email}) initiated deletion of user ${targetEmail} (${targetRole})`
      );
      ...
      if (targetUser.seeker_profile) {
        ...
        deletionSummary.details["application_notes_deleted"] =
          applications.length;
      ...
      if (targetUser.employer_profile) {
        ...
        deletionSummary.details["job_applications_deleted"] =
          totalApplicationsDeleted;
        deletionSummary.details["job_bookmarks_deleted"] =
          totalBookmarksDeleted;
        ...
        deletionSummary.details["managed_companies_deleted"] =
      ...
      deletionSummary.details["notifications_deleted"] =
      ...
      console.log(
        `✅ User ${targetEmail} (${targetRole}) permanently deleted by ${currentRole} (${currentUser.email})`
      );
    ...
    try {
      ...
      if (currentRole !== "Admin" && currentRole !== "Super Admin") {
        return res.status(403).json({
          success: false,
          message:
            "Access denied. Only Admin or Super Admin can reset passwords.",
        });
      }
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="info-box">
                ...
                ${
                  strengthWarning
                    ? `<p style="color: #f59e0b; margin-top: 8px;">⚠️ ${strengthWarning}</p>`
                    : ""
                }
              </div>
              ...
              <p style="margin-top: 20px;">If you did not request this password reset, please contact our support team immediately at <a href="mailto:${
            ...
            <div class="footer">
              ...
              <p style="margin-top: 10px;">
                <small>This password reset was performed by: ${
                  currentUser.email
                } (${currentRole})</small>
              </p>
            </div>
      ...
      // ========== AUDIT LOG ==========
      console.log(
        `📝 AUDIT: Password reset for user ${targetUser.email} (${targetRole})`
      );
      ...
      res.json({
        ...
        message: `Password reset successfully for ${targetUser.email}${
          emailSent
            ...
            : ". Email notification could not be sent, but password has been reset."
        }`,
        data: {
          user_id: userId,
    ...
    try {
      ...
      if (!seeker) {
        return res
          ...
          .json({
            success: false,
            message: "Only job seekers can access preferences",
          });
      }
    ...
    try {
      ...
      if (!seeker) {
        return res
          ...
          .json({
            success: false,
            message: "Only job seekers can update preferences",
          });
      }
      ...
      const preferences = await prisma.notificationPreference.upsert({
        ...
        update: {
          email_notifications:
            email_notifications !== undefined ? email_notifications : undefined,
          application_updates:
            application_alerts !== undefined ? application_alerts : undefined, // Map to correct field
          marketing_emails:
            marketing_emails !== undefined ? marketing_emails : undefined,
          push_notifications:
            push_notifications !== undefined ? push_notifications : undefined,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        await emailTransporter.sendMail({
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            job.employer.company_name
              ...
              <a href="${
                process.env.CLIENT_URL || "http://localhost:5173"
              }/applications" style="background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block; margin: 20px 0;">Track Your Application</a>
              <p>Best regards,<br><strong>Job Portal Team</strong></p>
            </div>
        ...
        console.log(
          `✅ Application confirmation email sent to: ${req.user!.email}`
        );
      } catch (emailError) {
      ...
      try {
        console.log(
          `📧 Sending notification email to employer: ${job.employer.user.email}`
        );
        ...
        await emailTransporter.sendMail({
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="padding: 30px; background: #f9fafb;">
              <h3>Hello ${job.employer.company_name} Team,</h3>
              <p>A new candidate has applied for <strong>${
                job.title
              }</strong>.</p>
              <div style="background: #e0e7ff; padding: 15px; border-radius: 8px; margin: 20px 0;">
                ...
                ${
                  coverLetter
                    ? `<p><strong>Cover Letter:</strong> ${coverLetter.substring(
                        0,
                        200
                      )}${coverLetter.length > 200 ? "..." : ""}</p>`
                    : ""
                }
              </div>
              <a href="${
                process.env.CLIENT_URL || "http://localhost:5173"
              }/employer/applications" style="background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block;">View Application</a>
              <p>Best regards,<br><strong>Job Portal Team</strong></p>
            </div>
          </div>
        `,
        ...
        console.log(
          `✅ New application notification email sent to employer: ${job.employer.user.email}`
        );
      } catch (emailError) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const applications = await prisma.jobApplication.findMany({
        ...
        include: {
          job: {
            include: { employer: true, industry: true, employment_type: true },
          },
          status: true,
        },
        orderBy: { applied_at: "desc" },
    ...
    try {
      ...
      if (!pendingStatus) {
        return res
          ...
          .json({
            success: false,
            message: "Application status configuration missing",
          });
      }
      ...
      try {
        console.log(
          `📧 Sending confirmation email to job seeker: ${req.user!.email}`
        );
        ...
        <body>
          ...
          job.employer.company_name
              ...
              <div style="background: #e0e7ff; padding: 15px; border-radius: 8px; margin: 20px 0;">
                ...
                <p style="margin: 5px 0;">💼 Type: ${
                  job.employment_type_id === 1 ? "Full-time" : "Part-time"
                }</p>
                ${
                  job.salary_range
                    ? `<p style="margin: 5px 0;">💰 Salary: ${job.salary_range}</p>`
                    : ""
                }
              </div>
              <p>The employer will review your application and update you on the status.</p>
          ...
          application.id
        }" class="button">View This Application</a>
              </div>
              <p>Best regards,<br><strong>Job Portal Team</strong></p>
            </div>
            <div class="footer">
      ...
      `;
        ...
        console.log(
          `✅ Application confirmation email sent to job seeker: ${
            req.user!.email
          }`
        );
      } catch (emailError) {
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div style="background: #e0e7ff; padding: 15px; border-radius: 8px; margin: 20px 0;">
                ...
                ${
                  coverLetter
                    ? `<p style="margin: 5px 0;">📝 Cover Letter: ${coverLetter.substring(
                        0,
                        200
                      )}${coverLetter.length > 200 ? "..." : ""}</p>`
                    : ""
              ...
              <div style="text-align: center;">
                <a href="${
                  process.env.CLIENT_URL || "http://localhost:5173"
                }/employer/applications" class="button">👁️ View Application</a>
              </div>
              <p>Best regards,<br><strong>Job Portal Team</strong></p>
            </div>
            <div class="footer">
...
app.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        console.log(
          `📧 Sending status update email to job seeker: ${application.seeker.user.email}`
        );
        ...
        <head>
          <style>
            ...
            .content { background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; }
            .status { display: inline-block; padding: 5px 15px; border-radius: 20px; font-weight: bold; background: ${
              statusColors[newStatus.status_name]
            }; color: white; }
        ...
        <body>
          ...
          employer.company_name
              ...
              ${
                message
                  ? `<div class="message-box"><strong>Message from employer:</strong><br>${message}</div>`
                  : ""
              }
              <p style="text-align: center;">
                <a href="${
      ...
      `;
        ...
        console.log(
          `✅ Status update email sent to job seeker: ${application.seeker.user.email}`
        );
      } catch (emailError) {
        console.error("❌ Failed to send status update email:", emailError);
      }
      ...
      await createNotification(
        ...
        `Your application for "${
          application.job.title
        }" has been ${newStatus.status_name.toLowerCase()}`,
        "application_update",
      ...
      res.json({
        ...
        message: `Application ${newStatus.status_name.toLowerCase()} successfully`,
      });
    } catch (error: any) {
      console.error("Error updating application status:", error);
      res.status(500).json({ success: false, message: error.message });
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div style="text-align: center;">
              <a href="${
                process.env.CLIENT_URL || "http://localhost:5173"
              }/notifications" class="button">Manage Notifications</a>
            </div>
            <p>Best regards,<br><strong>Job Portal Team</strong></p>
          </div>
          <div class="footer">
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const bookmark = await prisma.jobBookmark.create({
        data: { job_id: jobId, seeker_id: seeker.id },
      });
      res.json({
        success: true,
        data: bookmark,
...
app.get("/api/job-statuses", async (req: Request, res: Response) => {
  try {
    res.json({
      success: true,
      data: await prisma.jobPostStatus.findMany({ orderBy: { id: "asc" } }),
    });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          totalJobSeekers: await prisma.user.count({
            where: { user_type: { type_name: "Job Seeker" } },
          }),
          totalEmployers: await prisma.user.count({
            where: { user_type: { type_name: "Employer" } },
          }),
          totalAdmins: await prisma.user.count({
            where: {
              user_type: { type_name: { in: ["Admin", "Super Admin"] } },
            },
          }),
          totalJobs: totalJobs,
          ...
          closedJobs: await prisma.jobPost.count({
            where: { status: { status_name: "Closed" } },
          }),
          ...
          reviewedApplications: await prisma.jobApplication.count({
            where: { status: { status_name: "Reviewed" } },
          }),
          shortlistedApplications: await prisma.jobApplication.count({
            where: { status: { status_name: "Shortlisted" } },
          }),
          interviewApplications: await prisma.jobApplication.count({
            where: { status: { status_name: "Interview" } },
          }),
          acceptedApplications: await prisma.jobApplication.count({
            where: { status: { status_name: "Accepted" } },
          }),
          rejectedApplications: await prisma.jobApplication.count({
            where: { status: { status_name: "Rejected" } },
          }),
          ...
          totalViews:
            (await prisma.jobPost.aggregate({ _sum: { views_count: true } }))
              ._sum.views_count || 0,
          averageViewsPerJob:
            totalJobs > 0
              ? Math.round(
                  (
                    await prisma.jobPost.aggregate({
                      _sum: { views_count: true },
                    })
                  )._sum.views_count! / totalJobs
                )
              : 0,
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          // Monthly Trends
          jobsByMonth,
          applicationsByMonth: jobsByMonth.map((m) => ({
            month: m.month,
            count: m.applications,
          })),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const jobGrowth =
        lastMonthJobs > 0
          ? Math.round(
              ((newJobsThisMonth - lastMonthJobs) / lastMonthJobs) * 100
            )
          : newJobsThisMonth > 0
          ? 100
          : 0;
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // ========== SERVER HEALTH ==========
      const serverStartTime = parseInt(
      ...
      const recentApplications = await prisma.jobApplication.count({
        where: {
          applied_at: { gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) },
        },
      });
      ...
      if (storageUsage > storageTotal * 0.8) {
        alerts.push({
          level: "warning",
          message: `Storage usage at ${Math.round(
            (storageUsage / storageTotal) * 100
          )}%`,
          timestamp: new Date().toISOString(),
        });
      }
        ...
        data: {
          ...
          components: {
            ...
            storage: {
              ...
              total_formatted:
                (storageTotal / (1024 * 1024 * 1024)).toFixed(2) + " GB",
          ...
          server: {
            ...
            memory_heap_total_mb: Math.round(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (enableRegistration !== undefined)
        updateData.enable_registration = enableRegistration;
      if (emailVerification !== undefined)
        updateData.require_email_verification = emailVerification;
      if (defaultUserRole !== undefined)
        updateData.default_user_role = defaultUserRole;
      if (maxUsers !== undefined) updateData.max_users = maxUsers;
      if (autoApproveEmployers !== undefined)
        updateData.auto_approve_employers = autoApproveEmployers;
      if (allowSocialLogin !== undefined)
        updateData.allow_social_login = allowSocialLogin;

      // Job Settings
      if (requireJobApproval !== undefined)
        updateData.require_job_approval = requireJobApproval;
      if (maxJobsPerEmployer !== undefined)
        updateData.max_job_posts_per_employer = maxJobsPerEmployer;
      if (jobExpiryDays !== undefined)
        updateData.job_expiry_days = jobExpiryDays;
      if (allowJobEditing !== undefined)
        updateData.allow_job_editing = allowJobEditing;
      if (featuredJobPrice !== undefined)
        updateData.featured_job_price = featuredJobPrice;
      if (allowFreeJobs !== undefined)
        updateData.allow_free_jobs = allowFreeJobs;
      if (maxJobTags !== undefined) updateData.max_job_tags = maxJobTags;

      // Application Settings
      if (maxApplicationsPerSeeker !== undefined)
        updateData.max_applications_per_seeker = maxApplicationsPerSeeker;
      if (requireCoverLetter !== undefined)
        updateData.require_cover_letter = requireCoverLetter;
      if (requireResumeUpload !== undefined)
        updateData.require_resume_upload = requireResumeUpload;
      if (applicationCooldown !== undefined)
        updateData.application_cooldown = applicationCooldown;
      if (allowWithdrawApplication !== undefined)
        updateData.allow_withdraw_application = allowWithdrawApplication;
      if (notifyEmployerOnApplication !== undefined)
        updateData.notify_employer_on_application = notifyEmployerOnApplication;

      // Security Settings
      if (passwordMinLength !== undefined)
        updateData.password_min_length = passwordMinLength;
      if (requireUppercase !== undefined)
        updateData.require_uppercase = requireUppercase;
      if (requireNumbers !== undefined)
        updateData.require_numbers = requireNumbers;
      if (requireSpecialChars !== undefined)
        updateData.require_special_chars = requireSpecialChars;
      if (sessionTimeout !== undefined)
        updateData.session_timeout = sessionTimeout;
      if (maxLoginAttempts !== undefined)
        updateData.max_login_attempts = maxLoginAttempts;
      if (twoFactorAuth !== undefined)
        updateData.two_factor_auth = twoFactorAuth;
      if (lockoutDuration !== undefined)
        updateData.lockout_duration = lockoutDuration;
      ...
      if (notifyOnNewJob !== undefined)
        updateData.notify_on_new_job = notifyOnNewJob;
      if (notifyOnApplication !== undefined)
        updateData.notify_on_application = notifyOnApplication;
      if (notifyOnStatusChange !== undefined)
        updateData.notify_on_status_change = notifyOnStatusChange;
      if (weeklyDigest !== undefined) updateData.weekly_digest = weeklyDigest;
      if (adminAlertEmails !== undefined)
        updateData.admin_alert_emails = adminAlertEmails;
      ...
      if (smtpEncryption !== undefined)
        updateData.smtp_encryption = smtpEncryption;
      if (fromEmail !== undefined) updateData.from_email = fromEmail;
      if (fromName !== undefined) updateData.from_name = fromName;

      // API Settings
      if (googleClientId !== undefined)
        updateData.google_client_id = googleClientId;
      if (googleClientSecret !== undefined && googleClientSecret !== "")
        updateData.google_client_secret = googleClientSecret;
      if (linkedinClientId !== undefined)
        updateData.linkedin_client_id = linkedinClientId;
      if (linkedinClientSecret !== undefined && linkedinClientSecret !== "")
        updateData.linkedin_client_secret = linkedinClientSecret;
      if (recaptchaSiteKey !== undefined)
        updateData.recaptcha_site_key = recaptchaSiteKey;
      if (recaptchaSecretKey !== undefined && recaptchaSecretKey !== "")
        updateData.recaptcha_secret_key = recaptchaSecretKey;

      // Backup Settings
      if (autoBackup !== undefined) updateData.auto_backup = autoBackup;
      if (backupFrequency !== undefined)
        updateData.backup_frequency = backupFrequency;
      if (backupRetention !== undefined)
        updateData.backup_retention = backupRetention;
      if (logRetentionDays !== undefined)
        updateData.log_retention_days = logRetentionDays;

      // File Settings
      if (allowedFileTypes !== undefined)
        updateData.allowed_file_types = allowedFileTypes;
      if (maxFileSizeMb !== undefined)
        updateData.max_file_size_mb = maxFileSizeMb;
      ...
      console.log(
        `✅ System settings updated by Super Admin: ${req.user!.email}`
      );
        ...
        for (const admin of admins) {
          await prisma.notification.create({
            data: {
              ...
              message: `System maintenance mode has been ${
                maintenanceMode ? "enabled" : "disabled"
              } by ${req.user!.email}`,
              type: "system",
              created_at: new Date(),
...
app.post(
  "/api/super-admin/settings/test-email",
  authMiddleware,
  superAdminMiddleware,
  async (req: Request, res: Response) => {
    try {
      ...
      if (!settings || !settings.smtp_user || !settings.smtp_password) {
        return res.status(400).json({
          success: false,
          message:
            "SMTP configuration is not set up. Please configure email settings first.",
        });
      }
      ...
      await transporter.sendMail({
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="padding: 30px; background: #f9fafb; border-radius: 10px; margin-top: 20px;">
            ...
            <p>This is a test email from your <strong>${
              settings.site_name
            }</strong> system.</p>
      ...
      res.json({
        success: true,
        message: `Test email sent successfully to ${
  ...
  try {
    ...
    const jobs = await prisma.jobPost.findMany({
      ...
      include: {
        ...
        employment_type: { select: { type_name: true } },
      },
      orderBy: { created_at: "desc" },
      take: limit,
    });
    ...
    const formatted = jobs.map((job) => ({
      ...
      location:
        job.location || job.employer?.location || "Location not specified",
  ...
  try {
    ...
    for (const job of jobTitles) {
      if (job.title?.trim())
        searches.push({
          label:
            job.title.length > 25 ? job.title.slice(0, 22) + "..." : job.title,
          query: job.title.toLowerCase(),
    ...
    for (const loc of locations) {
      if (loc.location?.trim())
        searches.push({
          label: loc.location,
          query: loc.location.toLowerCase(),
          icon: "📍",
    ...
    // Remove duplicates
    const seen = new Set();
    const unique = searches.filter(
      (s) => !seen.has(s.label.toLowerCase()) && seen.add(s.label.toLowerCase())
    );
    ...
    res.json({
      success: true,
      data: [
        { label: "Remote", query: "remote", icon: "🌍" },
        { label: "Software Engineer", query: "software engineer", icon: "💻" },
      ],
    });
  }
});
...
app.get("/api/companies/top", async (req: Request, res: Response) => {
  try {
    ...
    const employers = await prisma.employerProfile.findMany({
      include: { jobs: { where: { status: { status_name: "Open" } } } },
    });
